...
圆角 capsule，Overview 必须保留 v16 command deck：mission board、下一步动作、主指标/覆盖判断、闭环 runway、运行信号区和最近 run 产物流，
...
command deck 需要滚动时不能被 hidden 裁切，mission、score cluster、loop 和最近 run 面板必须保持可读高度，不能在
...
- dashboard 共享交互原语位于 `ui.tsx`：标准命令使用 `ActionButton` / `CommandButton` /
  ...
  v16 value-first command deck 的 `overview-home-v16`、`overview-mission-board`、`overview-score-cluster`、
  `overview-workflow-row`、`overview-loop-panel`、`overview-flow-spine` 和 `overview-recent-card`，
  ...
  阻塞优先级面板、orbit 装饰、活动矩阵或低价值 mini chart wall；
