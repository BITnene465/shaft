...
圆角 capsule，Overview 必须保留 v12 two-band surface：主判断区、hero next action、四段闭环 spine、运行信号区、最近 run 产物流和下一步工作区入口，
...
two-band surface 需要滚动时不能被 hidden 裁切，hero、signal、route 和最近 run 面板必须保持可读高度，不能在
...
- dashboard 共享交互原语位于 `ui.tsx`：标准命令使用 `ActionButton` / `CommandButton` /
  ...
  v12 two-band surface 的 `overview-home-v12` / `overview-workband` / `overview-hero-board` /
  `overview-flow-spine` / `overview-signal-board` / `overview-recent-card` / `overview-route-panel`，不能回流旧 `overview-home-v6`、
