...
- `show-benchmark` 和 `show-service` 是 agent 读取单个 benchmark summary 与 model service 详情的稳定
  CLI；不要求 agent 扫描 benchmark artifact、读取 SQLite 或依赖 dashboard 前端 state。
