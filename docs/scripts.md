...
圆角 capsule，Overview 必须保留 v15 cockpit：下一步动作、主指标/覆盖判断、四段闭环 spine、运行信号区和最近 run 产物流，
...
cockpit 需要滚动时不能被 hidden 裁切，主判断、pulse、loop 和最近 run 面板必须保持可读高度，不能在
...
- dashboard 共享交互原语位于 `ui.tsx`：标准命令使用 `ActionButton` / `CommandButton` /
  ...
  `OptionChipButton`，任务队列表格选择使用 `SelectableTableRow`，对象行和可选卡片使用 `SelectableCardButton`，折叠面板入口使用
  ...
  v15 four-module cockpit 的 `overview-home-v15`、主判断指标、`overview-pulse-panel`、`overview-loop-panel`、
  `overview-flow-spine` 和 `overview-recent-card`，不能回流旧 `overview-home-v6`、`overview-home-v7`、
  `overview-home-v10`、`overview-home-v13`、阻塞优先级面板、orbit 装饰、活动矩阵或低价值 mini chart wall；
