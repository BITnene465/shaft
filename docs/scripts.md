...
圆角 capsule，Overview 必须保留 v8 decision surface：priority stage、command rail、hero next action、四段管线、行动入口、一个 operations surface 和最近 run 紧凑摘要，
...
- dashboard 共享交互原语位于 `ui.tsx`：标准命令使用 `ActionButton` / `CommandButton` /
  ...
  decision surface 的 `overview-home-v8` / `overview-command-center` / `overview-priority-stage` /
