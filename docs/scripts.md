...
圆角 capsule，ops / Run-Job-Service 活动矩阵 / 实时遥测必须保留在 signal deck，活动矩阵必须是 3 条 12 桶泳道，Overview 保留 4-7 个高价值可读 mini chart，
并同时包含多种微图表形态；图表主区域必须使用 capped masonry columns，默认 2 列、最多 4 列，不能退回纯 grid；Overview 不能出现 precision/recall/IoU 这类细指标文案，也不能回流 Notes、Label footprint、模型分布、Job 日历或 Scheduler 资源这类低价值总览面板；
...
- dashboard 共享交互原语位于 `ui.tsx`：标准命令使用 `ActionButton` / `CommandButton` /
  ...
  `OptionChipButton`，对象行和可选卡片使用 `SelectableCardButton`；紧凑 select、表单 select、
  number、color 和 toggle 控件位于 `controlPrimitives.tsx`。
...
- CLI 子命令统一通过 `eval_bench.cli._command_handlers()` 分发；新增子命令必须同时进入 parser 和
  ...
  `AGENT_STABLE_COMMANDS`。agent 可用 `list-agent-commands` 发现当前稳定命令面。
