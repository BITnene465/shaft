...
圆角 capsule，ops / 写入节奏 / 实时遥测必须压缩在 signal deck，写入节奏必须是 12 桶微型条，Overview 至少保留 20 个 mini chart，且不能出现
precision/recall/IoU 这类细指标文案；图表矩阵需要滚动时不能被 hidden 裁切，最近 run 行不能被面板高度拉伸。
