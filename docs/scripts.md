...
Rank Board 还会检查 facet rail 使用 `.rank-facet-button` 可点击 chip，点击后能把同一份高级检索状态置为 active，
防止核心排行榜退回静态 facet 摘要。
UI contract 还会锁住 Rank Board 表格第一分数列必须使用 active primary metric label / score，避免用户切换
主指标后表格仍固定展示 F1。
