...
圆角 capsule，Overview 必须保留 v17 decision-first command desk：ops board、下一步动作、F1 dial、四个决策 tile、闭环 runway、rank console 和最近 run 产物流，
...
- dashboard 共享交互原语位于 `ui.tsx`：标准命令使用 `ActionButton` / `CommandButton` /
  ...
  `overview-evidence-row`、`overview-loop-panel`、`overview-flow-spine` 和 `overview-recent-card`，
