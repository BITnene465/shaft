...
圆角 capsule，Overview 必须保留 v13 command console：主判断区、hero next action、F1/覆盖/backlog proof metric、四段闭环 spine、运行信号区、triage rail 和最近 run 产物流，
...
command console 需要滚动时不能被 hidden 裁切，now、live、loop 和最近 run 面板必须保持可读高度，不能在
...
- dashboard 共享交互原语位于 `ui.tsx`：标准命令使用 `ActionButton` / `CommandButton` /
  ...
  v13 command console 的 `overview-home-v13` / `overview-command-shell` / `overview-now-panel` /
  `overview-proof-strip` / `overview-live-panel` / `overview-loop-panel` / `overview-flow-spine` /
  `overview-triage-rail` / `overview-recent-card`，不能回流旧 `overview-home-v6`、
