...
- `list-agent-commands` 是 agent 稳定命令面的发现入口；run 初始化、prediction 文档校验和手动推进队列
  ...
  Dashboard 的 `GET /api/agent/commands` 和 `GET /api/agent/commands/{name}` 复用同一份 contract；
  每条 contract 的 `api_routes` 列出 HTTP 等价入口、method、path、query 参数和 body 形态。
  稳定 `show-*` 命令需要同步提供单对象 HTTP detail route；benchmark、run、job、job template、
  prompt template、service 和 saved comparison 都不能要求 HTTP agent 扫描分页列表后自行匹配 id。
