...
- dashboard 共享交互原语位于 `ui.tsx`：标准命令使用 `ActionButton` / `CommandButton` /
  ...
  `overview-evidence-row`、`overview-ops-signal`、`overview-telemetry-trace`、`overview-loop-panel`、`overview-flow-spine` 和 `overview-recent-card`，
  ...
  `overview-signal-stack`、阻塞优先级面板、orbit 装饰、活动矩阵或低价值 mini chart wall；
...
- `list-agent-commands` 是 agent 稳定命令面的发现入口；run 初始化、prediction 文档校验和手动推进队列
  ...
  Agent contract 是 CLI-first：`show-agent-command` 暴露 argv、参数、互斥组、副作用和输出 schema，
  不暴露 Dashboard route catalog，也不把 Dashboard API 做成 MCP 式能力发现层。Agent 需要机器可读失败时，
  在命令名前使用 `--json-errors` 或设置 `EVAL_BENCH_JSON_ERRORS=1`；stderr 会输出
  `ok=false`、`command`、`error_type` 和 `message`，exit code 仍保持非零。
  返回 `filters` 的稳定查询命令必须同步 `argument_semantics.filters` 和分页语义，说明 exact match、
  membership、全文 query 的字段范围；agent 不能靠解析自然语言 help 或手扫 store 推断高级检索行为。
  所有 `mutates_state=true` 的稳定命令也必须有非空 `argument_semantics`，明确 payload、lifecycle、
  service、report 或 comparison 参数语义；创建、删除、启动、停止、评估和对比命令不能只靠 argparse
  flag 列表让 agent 猜副作用边界。
  样本级列表同样必须在 CLI 和 Dashboard API payload 中回显 `filters`，保证 label / error 筛选和分页可复现。
- run note 是人类和 agent 共同维护的可变真源；`set-run-note` 和 `append-run-note` 都必须支持
  `expected_updated_at` 乐观并发保护，并在 `argument_semantics.note` 中暴露 run id、note/note-file、
  heading 和版本字段语义，避免 agent 通过手改 `note.json` 或无保护 append 覆盖人工线索。
- agent JSON 命令必须支持被 `head`、`jq`、日志采集器或分页器提前截断 stdout；CLI 应安静处理
  `BrokenPipeError`，不能把 traceback 混入机器可读输出。
- agent JSON 错误模式必须覆盖 argparse 参数错误和运行时异常，避免 agent 解析 Python traceback。
...
- 如果预测 JSON 已经在外部目录，可以用 `import-predictions --benchmark-id <benchmark> --prediction-root <dir> --run-id <run>` 导入为 Eval Bench run；它会按 benchmark split 的相对路径、image path 或 basename 对齐 prediction JSON，并在 benchmark 有 label index 时校验 `--target-label`，默认导入后立即运行 `evaluate-run`。Dashboard 的 Runs 页也提供默认折叠的 `Import prediction snapshot` 入口，提交后直接生成可打开的 run
- `resolve-target-labels` 和 Dashboard `GET /api/target-labels` 是 agent 创建 detection label 子任务前的稳定查询入口；它复用 benchmark summary、
...
- `show-job-template`、`show-prompt-template`、`show-job`、`show-run`、`show-benchmark`、
  `show-service` 和 `show-comparison` 是 agent 读取单个模板、job、run、benchmark summary、model service
  与 saved comparison 详情的稳定 CLI/API；不要求 agent 扫描 benchmark artifact、读取 SQLite、遍历
...
- `show-comparison` 和 `show-comparison-sample` 是 agent 读取已保存 comparison 和成对样本详情的稳定 CLI/API；
