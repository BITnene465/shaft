...
圆角 capsule，Overview 必须保留 v9 mission-control surface：priority stage、command rail、hero next action、四段管线、行动入口、一个 operations surface 和最近 run 紧凑 ticker，
...
防止核心排行榜退回静态 facet 摘要或漏接后端 facet。
...
- dashboard 共享交互原语位于 `ui.tsx`：标准命令使用 `ActionButton` / `CommandButton` /
  ...
  v9 mission-control surface 的 `overview-home-v9` / `overview-command-center` / `overview-priority-stage` /
  `overview-command-rail` / `overview-workbench` / `overview-ops-surface`，不能回流旧 `overview-home-v6`、
  `overview-home-v7`、`overview-command-deck`、`overview-focus-panel`、阻塞优先级面板、orbit 装饰或活动矩阵组件；
