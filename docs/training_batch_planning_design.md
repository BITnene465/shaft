...
当前 `training/loss.py` 返回每个 local microbatch 的 mean。这在以下情况不再正确：
...
具体实现可以通过经过校验的预计划 denominator、DDP scaling、显式聚合，或在 optimizer step 前对
累计梯度统一缩放完成，但必须用等价性测试锁定。不能把每个 microbatch mean 等权相加。

固定 batch 模式继续保持现有 HF 兼容路径。只有显式开启 cost-aware/dynamic batching 时才启用新的
...
- cost-aware 模式下，一个 optimizer step 的 sample 数允许变化，因此 sample plan budget 由 BatchPlan
  驱动，不再由固定 batch 公式唯一推导。
...
以下只是目标结构，**本设计阶段不加入 schema，也不允许 YAML 提前使用**：
...
- 定义 `ShaftSampleCost`、fingerprint 和离线/缓存入口。
- 增加 padding、token、vision patch、rank skew 指标。
- 用真实 Qwen3VL 数据拟合/验证成本排序，不改变 batch 行为。

### Phase 1：固定样本数的成本分桶

- 新增有界 planning window 和 2D cost bucketing。
- 每个 local microbatch 样本数仍等于现有配置，避免同时修改 loss/global batch 语义。
- BatchPlan 在全局 DP rank 层平衡成本。
- 完成确定性、mixing multiset、resume 和 DDP tests。

### Phase 2：动态 cost-budget microbatch

- 允许 local microbatch 样本数变化。
- 引入 optimizer-batch plan 和全局 numerator/denominator loss。
