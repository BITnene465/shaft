...
状态：**Phase 0-2 与共享 mmap CostPlan 已实现；Phase 3-4 待实现**
...
optimizer-batch global numerator/denominator，以及动态 cost-budget local microbatch。sequence packing 与
context parallel 仍仅保留本文档中的目标契约。
运行时 cost provider 只编排数据：模型 token expansion 由 `ProcessorPolicy` 估算，target/EOS/causal
supervision 由 `Template` 估算；模型 policy 还提供覆盖 estimator 全部依赖状态的版本化 cost-semantics
signature，避免模型或 processor 升级后出现第二套成本语义或 stale CostPlan。
...
- optimizer step；若未来允许 step 内 checkpoint，再增加 microstep cursor；
...
会 fail fast。Phase 2 动态计划同样只在 optimizer-step checkpoint 边界恢复，并把 target、hard budgets、
selected horizon 和 DP/GA topology 纳入签名。把已有 run 延长为更多 steps 暂不视为 exact resume，应从
权重启动新 run；可扩展 horizon/window cursor 仍是独立后续能力。
所有训练都必须在 dataset、base model 与 PEFT adapter 装配前初始化 `experiment.seed`；只依赖 Trainer
初始化 seed 会让 fresh full/LoRA 参数先分叉。CUDA 上的 bitwise exact resume/fresh reproduction 还要求
执行 kernel 本身确定；`train.full_determinism=true` 用于该验收，默认非确定性 FlashAttention backward
的数值差异不能归因给 BatchPlan。SFT 训练内 eval 必须是 RNG observer：评估调用前后恢复 Python、
NumPy、Torch CPU 和当前 rank CUDA RNG，避免 persistent worker 的 iterator 生命周期让
uninterrupted/resume 在下一训练步读取不同主进程随机状态。随机 worker-side eval transform 必须从
sample/global-step 派生独立确定性随机数。
...
Phase 2 已开放以下动态字段；`packing` 子块仍属于 Phase 3，尚不能配置：
...
- `per_device_train_batch_size` 在 `strategy=fixed` 下保持原语义；dynamic 模式下只作为未显式配置
  `max_samples_per_microbatch/optimizer target` 的兼容默认值，normalize 会把默认值实体化，preflight 与
  planning signature 会展示最终 resolved target/horizon。
- 动态字段已同步 schema、normalize、配置文档和消费测试；packing 字段正式落地前不得进入 YAML。
- Phase 2 的动态执行拓扑限定为 DDP；FSDP/DeepSpeed 仍可用于 fixed batch，但动态 DataLoader 合约在
  完成独立 smoke、loss parity 与 resume 验收前不开放。
- greedy 分配未找到结果时会进入确定性的 bounded feasibility search。它包含 token/cardinality/vision
  的安全 lower bound，并使用显式 stack 的 iterative DFS，不受 Python recursion limit 约束；但分桶
  问题本身是 NP-hard，若达到搜索预算会抛出独立的
  `ShaftDynamicPartitionSearchBudgetExceeded`（表示“可行性未知”），不会误报成“已证明不可分配”。
...
- **已实现**：local microbatch 样本数可变，但每个 optimizer step 的 global microstep 数仍固定为
  `gradient_accumulation_steps`。
- **已实现**：`ShaftDynamicBatchPlanningContract` 统一 optimizer target、DP/GA slots、每步 draw cap 和
  SamplePlan horizon；planner 只消费连续 draw prefix，不改变 mixing multiset。
- **已实现**：sample/padded LLM token/vision patch 三类硬预算、跨 rank 成本配对、rank-zero 全计划
  preflight，以及 `BatchSamplerShard(even_batches=false)` 的真实 DDP 分片。
- **已实现**：sample target、supervised-token target、实际 sample throughput、完整 planning signature 和
  optimizer-step 边界 exact resume；旧 fixed signature fingerprint 保持兼容。
- **已实现**：不同 rank/cardinality + GA>1 的 global loss/gradient 与单进程合批参考一致。
- **已实现**：greedy fallback 使用 iterative bounded DFS；8 slots / 1008 samples 的可行深度反例形成
  `2/2/2/2/250/250/250/250`，不再泄漏 `RecursionError`。
- **已实现**：resume 的最终 loss/steps/s/samples/s 按本次 invocation 的实际 optimizer-step sample counts
  计算；token target 启动 preflight 显式展示 optimizer-batch sample min/max/avg。
- **已验收**：CUDA 0/1 Qwen3-VL-4B LoRA DDP 的 2-step train/eval/save 与 checkpoint-1 resume。
  local cardinality 为 `2/2/7/3`、`2/2/3/7`；hard caps、712 个 adapter tensors、optimizer/scheduler、
  两 rank RNG、planning signature 和最终 adapter hash 全部逐值一致，两个导出均通过 validate。bitwise
  对照显式开启 `train.full_determinism`，eval RNG observer 覆盖 persistent workers。
- 当前范围只支持 SFT train + DDP；eval 保持 fixed batch，FSDP/DeepSpeed 动态路径尚未开放。全训练期
  动态 BatchPlan sidecar、离线 plan inspect 和
  profiler 驱动的 compute/memory cost 拟合仍是运维/性能后续项。
...
- 当前只在 optimizer-step checkpoint 边界精确恢复；未来若支持 step 内 checkpoint，microstep cursor
  必须持久化并恢复到相同位置。
