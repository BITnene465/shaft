...
本文保留从项目首页迁入的详细命令、配置示例与能力边界。所有 shell 命令均从仓库根目录执行；完整字段定义以[配置参考](config_reference.md)为准。

[返回首页](../README.md) · [文档索引](README.md)

## 训练数值与保存精度

- `train.loss_normalization` 支持 `global_token`（默认）、`rank_token` 和 `microbatch_token`。
  更换每卡 batch、GPU 数量或梯度累积时，必须重新核对归一化语义，见 [SFT loss 归一化](sft_loss_normalization.md)。
- `train.liger` 的 `fused_linear_ce`、`rms_norm`、`swiglu` 分别开启，不由安装依赖自动启用。
  适用模型与后端见[配置参考](config_reference.md#trainliger)。
- `model.torch_dtype` 控制参数加载，`train.bf16/fp16` 控制 AMP，`train.export_dtype` 控制模型导出。
  导出默认 `preserve`，可选择 `float32`、`bfloat16`、`float16`；可恢复的周期 checkpoint 保留原精度。
  当前显式 dtype 转换仅支持未量化 full 模型的单卡/DDP 路径，不支持 PEFT/FSDP/DeepSpeed 转换。
- `save_only_model: true` 适合只需部署权重的训练，不保存优化器状态，也不能用于精确断点续训。
...
所有未完成项统一见 [`docs/TODO.md`](TODO.md)。
...
[`docs/architecture.md`](architecture.md)、[`docs/config_reference.md`](config_reference.md) 与
...
logical samples；默认 `global_token` 下，loss 按该 optimizer frame 内跨 rank 的真实有效 token 归一化。
选择其它 `loss_normalization` 时，以对应归一化合同为准。
...
FSDP/ZeRO-3 的完整权重门禁及配置约束见 [`docs/config_reference.md`](config_reference.md)。
...
reasoning effort；完整模板名和 fail-closed 数据合同见 [`docs/config_reference.md`](config_reference.md)。
...
完整边界见
[`docs/training_batch_planning_design.md`](training_batch_planning_design.md) 与
...
- stage timeout 使用贯穿重试、backoff 和后端 I/O 的同一个绝对 deadline；pipeline 也接受 cooperative
  ...
  详细能力边界见 [docs/infer.md](infer.md)。
