...
- 启用 `add_eos_token` 时，只有完整 target 才补 EOS；被 `max_length` 强行截断的 target 不补 EOS，
  避免把半截 JSON 教成合法结束。
- Banana SFT 训练配置设置 `data.max_length: 4096`，避免超长 full-image JSON target 直接进入极慢路径。

### 回归测试

- `uv run pytest -q tests/test_collator.py::test_sft_collator_truncates_target_tokens_without_eos_when_over_max_length`
...
- 截断只是 runtime 防线，不是理想数据策略；dense grounding 后续应优先按 instance 数拆分 target 或过滤超限
  full-image 样本，但 runtime token 上限仍应保留。
...
## 2026-05-26: Eval Bench 总览主板存在空洞与重复面板

### 现象

总览页主操作板左侧留出大面积空白，下方又单独放置“闭环推进”面板，和主板中的报告覆盖、预测积压、
运行压力等信息重复。1440×900 首屏下，用户需要同时扫描两个相似区域，页面看起来不像总控工作台。

### 根因

上一轮总览重构把实时状态、闭环进度和最近产物拆成多个区域，但没有按首屏空间重新合并信息层级。
`OverviewFlowSpine` 仍停留在底部 evidence row，导致主操作板只负责状态抬头和 action card，
没有承担足够的工作流可视化。

### 影响范围

- 影响 Dashboard 总览页的信息价值密度、首屏空间利用和滚动体验。
- 不影响 CLI、job preflight、run manifest、evaluation metric 或 Rank Board 排序语义。
- 这是 Web UI 信息架构与布局问题，不是 eval / metric 计算问题。

### 修复方式

- 将 `OverviewFlowSpine` 移入主操作板左侧，让基准样本、预测产物、评估报告、报告评分在主决策区域内形成实时流线。
- 删除底部重复的“闭环推进”面板，只保留宽版 `Latest Runs` 产物轨道。
- 调整暗色主板中的 flow node 样式和 hover 状态，最近 run 列表改为两列网格，大屏更舒展，小屏回落为纵向滚动。
- 通过 Playwright 分别检查 1440×900 和 390×844 视口截图。
...
- 总览页新增面板前必须先判断是否和主操作板、最近产物轨道、Rank Board 页面职责重复。
- 首屏必须以真实状态流、队列风险、报告覆盖和最近产物为核心，不再用低价值说明块填充空间。

## 2026-05-26: Eval Bench detection metric profile 主指标命名仍是旧语义

### 现象

Rank Board API、CLI 和前端都已经默认使用 `f1_iou50` 作为主指标，但 `metric_profiles.py`
里的 `detection_iou_v1.primary_score` 仍写成 `precision_recall_iou50`。这不是当前排序 bug，
但会让后续新增入口或文档读取 metric profile 时误以为 detection 主指标不是 F1。

### 根因

Rank Board 主指标从早期 `score` / precision-recall 组合语义收敛到 `f1_iou50` 后，
store、CLI 和前端已经更新，但 profile 元数据字段没有同步收口。

### 影响范围

- 影响 metric profile 元信息和后续扩展入口的语义一致性。
- 不影响当前已生成 report、Rank Board 排序或 weighted scheme 计算。
- 这是 metric 元数据一致性问题，不是模型能力问题。

### 修复方式

- 将 `detection_iou_v1.primary_score` 改为 `f1_iou50`。
- 在 `test_eval_semantics.py` 中锁定 detection 默认 profile 的 `primary_score`。

### 回归测试

- `PYTHONPATH=projects/eval_bench .venv/bin/python -m pytest -q projects/eval_bench/tests/test_eval_semantics.py`
- `PYTHONPATH=projects/eval_bench .venv/bin/python -m pytest -q projects/eval_bench/tests/test_dashboard.py::test_dashboard_rank_board_defaults_to_f1_and_supports_weighted_scheme projects/eval_bench/tests/test_cli.py::test_cli_rank_board_outputs_agent_ready_json`

### 后续防线

- 新增 metric profile 字段时，必须确认它和 Rank Board 默认主指标、CLI 输出 schema、前端文案保持一致。
- 不再使用含糊的 precision/recall 组合名表示 detection 主指标。
...
## 2026-05-26: Eval Bench init-run 未读取 prompt metadata target labels

### 现象

`resolve-target-labels`、job preflight、worker 和 evaluator 都会读取 prompt template metadata 中的
`target_labels`，但 CLI `init-run` 只看显式 `--target-label`、legacy prompt id 和 task。
agent 使用自定义 prompt template 直接初始化 run 时，run manifest 可能不会继承该 prompt 的 label 子任务范围。

### 根因

`init-run` 是早期轻量 manifest 创建入口，尚未接入 prompt template registry；而后续 job/preflight 路径已经把
prompt metadata 作为评估语义真源之一。这让同一个 prompt 在不同创建入口下得到不同的 `target_labels_source`。

### 影响范围

- 影响 CLI/agent 直接使用自定义 prompt template 初始化 detection label 子任务 run。
- 不影响已有 report 计算、Rank Board 默认 F1、weighted scheme、Dashboard UI 或 viewer。
- 这是 eval / data 入口语义一致性问题，不是模型能力问题。

### 修复方式

- `init-run` 在解析 target label policy 前读取 `EvalBenchDatabase.get_prompt_template(prompt_id)`。
- 将 prompt template 的 `metadata.target_labels` 传给 `resolve_target_label_policy`，保持显式参数优先。
- 将读取到的 prompt metadata 写入 `spec.prompt.metadata`，让 evaluator 可以复现同一语义。
- 新增自定义 prompt metadata 回归测试，并把内置 prompt 的 source 更新为 `prompt_metadata`。
- README 和 `docs/scripts.md` 同步说明 `init-run` 会读取 prompt template registry。

### 回归测试

- `PYTHONPATH=projects/eval_bench .venv/bin/python -m pytest -q projects/eval_bench/tests/test_cli.py::test_init_run_cli_infers_target_labels_from_prompt_policy projects/eval_bench/tests/test_cli.py::test_init_run_cli_uses_custom_prompt_metadata_target_labels projects/eval_bench/tests/test_cli.py::test_init_run_cli_rejects_unknown_target_label_when_benchmark_has_index`
- `PYTHONPATH=projects/eval_bench .venv/bin/python -m pytest -q projects/eval_bench/tests/test_cli.py`

### 后续防线

- agent-facing CLI 创建入口必须读取同一套 prompt registry 和 label policy，不能只依赖 prompt id 字符串规则。
- 新增 prompt metadata 字段如果会影响评估语义，必须进入 init/preflight/worker/evaluator 的同源解析链路。
...
可以走 HTTP；后续补上 `api_routes` 后，`show-job`、`show-run`、`show-benchmark`、`show-service`、
`show-job-template`、`show-prompt-template` 和 `show-comparison` 又只能通过列表 API 扫描，缺少
单对象 detail route。

### 根因

Agent contract 的真源只覆盖 CLI argv 组合，没有把 HTTP route 视为同一能力面的可发现属性。
Dashboard API 也没有暴露与 CLI discovery 等价的 contract route；第一次修复只登记了已有 HTTP
入口，没有审计所有稳定 `show-*` 命令是否具备单对象 API。这是 agent 操作面自描述和 detail
查询缺口，不是模型能力问题，也不是 eval / codec / metric / data 误判。

### 影响范围

- 影响 HTTP agent 对 rank-board、target label resolution、run note、jobs、runs、benchmarks、
  services 和 comparisons 等 API 的稳定发现。
- 影响只想读取单个 benchmark/run/job/service/template/comparison 的 HTTP agent；没有 detail route 时，
  agent 需要拉取分页列表再自行匹配 id。
...
- Dashboard 补齐 `GET /api/benchmarks/{benchmark_id}`、`/api/runs/{run_id}`、`/api/jobs/{job_id}`、
  `/api/job-templates/{template_id}`、`/api/prompt-templates/{prompt_id}`、`/api/services/{service_id}`、
  `/api/comparisons/{comparison_id}`。
- CLI 和 Dashboard 测试覆盖 `resolve-target-labels`、`rank-board`、`dashboard-state` 与各个
  `show-*` detail route contract。

### 回归测试

- `cd /home/tanjingyuan/code/arrow-vlm && .venv/bin/python -m pytest -q projects/eval_bench/tests/test_cli.py::test_cli_lists_agent_stable_commands projects/eval_bench/tests/test_cli.py::test_cli_shows_single_agent_command_contract projects/eval_bench/tests/test_dashboard.py::test_dashboard_api_exposes_store_state projects/eval_bench/tests/test_dashboard.py::test_dashboard_resolves_target_labels_for_agents projects/eval_bench/tests/test_dashboard.py::test_dashboard_exposes_service_registry projects/eval_bench/tests/test_dashboard.py::test_dashboard_exposes_pairwise_comparison`
...
- 新增稳定 `show-*` CLI 时必须优先提供单对象 HTTP detail route；禁止要求 HTTP agent 扫列表再匹配 id。
...
总览页已经删除旧 hero slogan，但局部状态仍残留“等待报告进入排行”“主指标 F1 可排行”和 `rankable`
这类入口式文案。它们在闭环节点、当前最佳或小状态位里出现时，仍会把首页价值表达拉回“能点去某页”，
而不是让用户扫读 F1 报告、覆盖率、队列和最近产物。

### 根因

前一次修复只禁止了最显眼的“可以看排行 / 查看排行榜”类文案，没有把派生状态文案纳入 contract。
Overview 的跳转目标和显示文案没有分层：组件可以继续链接到 Rank Board，但可见文本应该表达数据状态，
不能表达导航动作。这是前端信息架构问题，不是模型能力问题，也不是 eval / codec / metric / data 误判。

### 影响范围

- 影响 Overview 首屏的专业感、信息密度和决策信号一致性。
- 不影响 rank-board 默认 F1、weighted rank scheme、report 计算、store schema 或 Dashboard API。

### 修复方式

- 将当前最佳空态改为“暂无 F1 报告”，闭环末端从“排行榜”改为“主指标”，值和说明只表达 F1 报告状态。
- 将 `rankable` / `F1 ready` 状态改为中文报告数量和“已评分”，总览姿态文案只描述报告数量、闭环覆盖和最佳
  F1 状态，不出现“可以看排行”或“可进入排序”这类入口式价值表达。
- layout smoke 和 UI contract 增加残留文案禁用项，覆盖“等待报告进入排行”“主指标 F1 可排行”“从样本到排行”、`rankable` 和 `F1 ready`。
- README、架构文档和脚本文档同步区分 Realtime Trace 的粗粒度资源占用与禁止回流的 scheduler 资源明细面板。
...
- Overview 文案可以暗含跳转目标，但不能把“能去 Rank Board”作为大字或状态价值。
- 新增总览状态文案必须被 layout smoke 的 body text 禁用集合覆盖，避免入口式文案换位置回流。
...
## 2026-05-27: Eval Bench suite split 与对比样本索引收口

### 现象

Layout eval 可视化排障时暴露出两类同源风险：suite benchmark 如果缺少真实 `suite` split manifest，
旧解析逻辑会回退到顶层 `manifest_path` 或按字母序取第一个 named split；Compare 页虽然报告中记录了
`baseline_index` / `candidate_index`，但打开样本详情时只传一个 `sample_index`。

### 根因

Benchmark split 解析仍保留旧单 split 兼容兜底，导致 broken suite 不会尽早失败。对比报告的样本对齐真源是
`json_path` key 和左右 index，但 dashboard API、CLI 与前端链接只消费了单个 index，形成了第二套弱语义。

### 影响范围

- 影响 Eval Bench 的 benchmark suite、job preflight、worker/evaluator split 选择和 prediction import。
- 影响 Compare 页在不同 split 或不同排序 run 之间打开样本详情时的左右样本对齐。
- 不改变 metric 计算公式、prediction parser 或 raw_data copy 格式。

### 修复方式

- `resolve_benchmark_split_path()` 和 `resolve_benchmark_split_name()` 改为严格要求 declared split；
  `split_manifests` 存在时，不再回退到顶层 `manifest_path` 或排序第一个 split。
- job preflight 将缺失 split manifest 从 warning 提升为 error，worker prepare-run 增加 benchmark label
  校验，避免绕过 preflight 后生成语义错误的 run manifest。
- Comparison sample detail API/CLI/前端链接支持 `baseline_index` 和 `candidate_index`，只在左右样本都存在时
  提供可点击详情。

### 回归测试

- `uv run ruff check projects/eval_bench/eval_bench/benchmark.py projects/eval_bench/eval_bench/job_spec.py projects/eval_bench/eval_bench/comparison.py projects/eval_bench/eval_bench/dashboard.py projects/eval_bench/eval_bench/cli.py projects/eval_bench/eval_bench/worker.py projects/eval_bench/tests/test_schema.py projects/eval_bench/tests/test_job_spec.py projects/eval_bench/tests/test_evaluator.py projects/eval_bench/tests/test_cli.py projects/eval_bench/tests/test_dashboard.py`
- `uv run pytest -q projects/eval_bench/tests`
- `cd projects/eval_bench/frontend && npm run test:formatters && npm run build`

### 后续防线

- 新增 benchmark suite 时，顶层 `split` 必须能在 `split_manifests` 中解析到同名真实 manifest。
- 新增跨 run 样本跳转时，必须携带对齐后的左右 index 或样本 key，不能默认认为两个 run 的 sample index
  表示同一个 GT 样本。

## 2026-05-27: Banana v2.4 评测口径与数据扩增防线

### 现象

新一轮评测验收指出 `point_arrow` eval、grounding pixel budget、grounding blur 扩增和 SFT 长度处理存在
口径漂移风险：point eval 可能继续用 raw full image，grounding 主评测默认仍是低分辨率预算，blur full
会把 full image 数据翻倍，SFT dense target 如果被截断后补 EOS，会把半截 JSON 教成合法结束。

### 根因

eval benchmark 构建、prompt template 默认值、structured 数据生成和训练 collator 分别维护了局部默认；
这些默认没有共同约束到“训练/val SFT 口径”和“主评测口径”。

### 影响范围

- 影响 Banana v2.4 eval benchmark 的 `point_arrow` split 和 grounding 主分。
- 影响 grounding structured train 的 full/blur 比例。
- 影响 SFT 训练时 dense target 是否被完整监督。
- 影响 Eval Bench report 的诊断可解释性。

### 修复方式

- Banana benchmark 脚本改为从 `data/point_arrow/structured/val.jsonl` 生成 crop 级 `point_arrow`
  benchmark split。
- Eval Bench 默认 prompt/job template 的 `max_pixels` 改为 `2000000`。
- grounding structured 构建新增 `--full-blur-ratio`，默认只把少量 full row 替换为 blur，不再 clean full
  后额外复制一份 blur。
- evaluator report 增加 empty prediction rate、pred/gt ratio、输出 char/token 长度分布和 dense 样本分桶。
- SFT collator 在 `prefix_tokens + target_tokens + eos > max_length` 时允许按预算截断 target，但截断后不补
  EOS；完整 target 仍按 `add_eos_token` 追加 EOS。

### 回归测试

- `uv run ruff check projects/eval_bench/eval_bench/evaluator.py projects/eval_bench/eval_bench/worker.py projects/eval_bench/eval_bench/prompt_templates.py projects/eval_bench/eval_bench/job_spec.py projects/eval_bench/tests/test_evaluator.py projects/eval_bench/tests/test_schema.py scripts/tasks/build_grounding_structured.py scripts/tasks/create_banana_v2_4_eval_benchmark.py src/shaft/template/base.py tests/test_collator.py`
- `uv run pytest -q projects/eval_bench/tests/test_evaluator.py projects/eval_bench/tests/test_schema.py projects/eval_bench/tests/test_job_spec.py projects/eval_bench/tests/test_cli.py::test_init_run_cli_infers_target_labels_from_prompt_policy projects/eval_bench/tests/test_cli.py::test_cli_manages_job_and_prompt_templates_for_agents tests/test_collator.py::test_sft_collator_truncates_target_tokens_without_eos_when_over_max_length`

### 后续防线

- `point_arrow` eval 必须保持 crop 级 benchmark；raw full image 只能作为 raw truth，不是 point eval 样本。
- `max_pixels=1000000` 只能用于 stress eval，不作为 grounding 主分。
- 数据重建后必须检查 train `full_image + full_image_blur` 约等于 covered source 数量，val 不增强。
- 长度审计应在训练前跑真实 processor；超限样本要过滤、记录或拆 crop。

## 2026-05-27: Banana v2.4 px1m mixfix 仍出现直接空数组预测

### 现象

`banana_v2_4_px1m_mixfix_last` 的 `checkpoint-5684` 在 Banana v2.4 eval-bench 上仍有大量 raw output 直接为 `[]`。这些不是 codec 解析失败：

- `grounding_arrow`: 400 张中 258 个 raw output 为 `[]`，其中 233 张 GT 中实际有 arrow。
- `grounding_layout`: 380 张中 38 个 raw output 为 `[]`，其中 37 张 GT 中实际有 layout target。
- `grounding_shape`: 380 张中 147 个 raw output 为 `[]`，其中 128 张 GT 中实际有 shape。
- `grounding_icon_image`: 380 张中 188 个 raw output 为 `[]`，其中 144 张 GT 中实际有 icon/image。
- `grounding_shape_arrow`: 380 张中 73 个 raw output 为 `[]`，其中 65 张 GT 中实际有 shape/arrow。

### 根因

本轮训练只调整了 catalog mix weight，并从上一轮 `checkpoint-7320` 继续训练；但实际 SFT 数据仍是旧的 grounding 派生产物，`full` 与 `full_blur` 仍 1:1 共存。例如 `grounding_arrow` train 中 `full=10221`、`full_blur=10221`，不符合后来约定的“blur 只占少量，full+blur 总量约等于 full 体量”。因此新训练没有真正消除旧数据分布中的 conservative bias，只是在更高 arrow/layout 权重下继续微调。

同时，prompt 明确允许 `Return [] when no target object is visible`，并且训练集中仍有合法空目标样本：`grounding_arrow` 7.30%、`grounding_shape` 7.78%、`grounding_icon_image` 11.78%、`grounding_shape_arrow` 5.69%、`grounding_layout` 2.00%。结合子任务检测的排他性提示和小目标/密集目标难度，模型形成了高 precision、低 recall 的保守策略：非空预测样本的定位质量较高，但遇到不确定样本会直接选择合法 `[]`。

### 影响范围

- 主要影响 grounding 子任务 recall，尤其 `grounding_arrow`、`grounding_shape`、`grounding_icon_image`。
- 不是 eval parser 或 prompt 不一致导致；训练与评测 prompt 已核对一致。
- 不是全部 `[]` 都错误；例如 `grounding_arrow` val 中 25 张 GT 本身没有 arrow，输出 `[]` 是正确负样本。

### 修复方式

- 重建 grounding structured/SFT，确保 `full_blur_ratio` 生效，不再用旧的 full/full_blur 双份数据训练。
- 训练前增加 derived data 审计：检查每个 grounding task 的 view_type 分布、空目标比例、target 长度分布和超长截断比例。
- 下一轮训练应优先提高 positive recall：减少或单独控量 hard negative，保持 blur 低比例，并考虑将“空目标”样本单独 bucket/weight，而不是与普通正样本同权混入。

### 回归测试

- 手工统计 `eval_bench_store/runs/banana_v2_4_px1m_mixfix_last__*/raw_outputs/images/*.txt`，确认空预测是 raw `[]`。
- 核对 `data/grounding_*/sft/train.jsonl` 与 eval run manifest 中的 prompt，一致。
- 统计 `data/grounding_*/sft/train.jsonl` 的 target `[]` 比例与 view 分布，确认本轮训练仍使用旧 full/full_blur 双份产物。

### 后续防线

- 修改数据生成脚本后，必须重建对应 structured/SFT；只改训练 mix weight 不会改变已有派生数据分布。
- 每次长训练前输出并保存 data audit 摘要，至少包含 view_type rows、empty target rows、target length p99/max、token truncation bucket。
- Eval report 中的 empty prediction rate 必须按 `GT empty` 和 `GT non-empty` 分开看，避免把合法负样本和错误空预测混在一起。

## 2026-05-27: Grounding blur 与 hard negative 派生数据重建

### 现象

上一版 grounding train 派生数据仍保留旧口径：每个 clean full-image row 额外复制一个
`full_image_blur` row，且 hard negative 使用 `0.04` 目标比例。该分布会继续放大像素退化样本和空目标样本，
不利于修复模型在 eval 中过度输出 `[]` 的问题。

### 根因

生成脚本已经支持 `--full-blur-ratio`，但默认值和已生成数据没有完全对齐当前训练策略；skill ref 中也仍有
“train additionally keeps one full-image blur row per covered source”的旧描述。

### 影响范围

- 影响 `grounding_arrow`、`grounding_layout`、`grounding_shape`、`grounding_icon_image`、
  `grounding_shape_arrow` 的 structured/SFT train 数据。
- 不影响 raw 数据、grounding val、point_arrow 数据。

### 修复方式

- `scripts/tasks/build_grounding_structured.py` 默认改为 `--full-blur-ratio 0.5` 和
  `--negative-ratio 0.008`。
- 用 `--clean --workers 20 --seed 42 --full-blur-ratio 0.5 --negative-ratio 0.008` 重建五个
  grounding structured 数据集。
- 用 `scripts/tasks/build_sft_from_structured.py --workers 30 --clean` 重建五个 grounding SFT 数据集。
- 更新 `shaft-data-manager` skill ref，明确 blur 是 full-image replacement subset，默认最多覆盖一半
  covered train sources；hard negative 默认目标比例为 `0.008`。
- 重建后 grounding row count 下降，重新调整本地 `configs/data/banana_v2_4.yaml` 中 `point_arrow`
  weight 到 `4.5`，使 2 epoch 约覆盖 `0.60x` point 数据。

### 回归测试

- `uv run ruff check scripts/tasks/build_grounding_structured.py`
- 校验五个 grounding task 的 structured/SFT train/val 行数一致。
- 校验 train `full_image + full_image_blur` 等于 covered train source 数，val 仍只有 clean
  `full_image`。
- 当前 mix 估算：2 epoch 约 `4880` optimizer steps，`point_arrow` 覆盖约 `0.601x`。

### 后续防线

- 每次重训前必须重新审计 derived data 的 view 分布，特别是 `full_image_blur` 和
  `hard_negative_crop`。
- 如果后续调整 hard negative 或 blur 比例，必须同时更新脚本默认值、生成 README 和 skill ref。

## 2026-05-28: 删除错误的 shape+arrow grounding 子任务

### 现象

`grounding_shape_arrow` 把 `shape` 和 `arrow` 合并成一个 detection 子任务，但该任务不是当前训练目标，
会额外引入错误的任务边界和评估 slice。

### 根因

早期为了覆盖组合标签，生成脚本、SFT 转换、训练 catalog、prompt registry 和 Eval Bench slice 中都加入了
`grounding_shape_arrow`。后续任务定义收敛为两类能力：grounding detection 子任务和 `point_arrow`
关键点任务；shape+arrow 组合任务没有独立业务语义。

### 影响范围

- 影响 grounding structured/SFT 派生产物、训练 mix、eval-bench prompt seeding 和 benchmark split 推断。
- 不影响 raw 数据；已有历史 eval 记录仍可作为旧实验记录解读。

### 修复方式

- 从 grounding structured 默认任务列表、SFT 转换任务列表、banana v2.4 catalog 和 SFT eval 配置中删除
  `grounding_shape_arrow`。
- 删除 `configs/prompts/grounding_shape_arrow.yaml` 和派生目录 `data/grounding_shape_arrow`。
- 从 Eval Bench 默认 prompt registry、legacy label policy、prompt path resolver 和 Banana slice 推断中删除
  shape+arrow 映射。
- 更新 `shaft-data-manager` skill ref，固定当前 grounding 子任务为 arrow、layout、shape、icon/image。

### 回归测试

- 重新 grep live 代码、配置、prompt 和 skill ref，确认除历史开发日志外不再引用该任务。
- 重新生成 SFT 数据时，默认任务列表不再产出 `grounding_shape_arrow`。

### 后续防线

- 新增组合任务前必须先确认它是独立训练/评估语义，而不是把多个 label 临时拼成一个任务。
- Eval Bench 的 prompt seeding 和 benchmark split 必须跟训练任务列表同步维护。
