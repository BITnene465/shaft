...
同类问题也存在于若干 router 文本入口：页面直接写 `Link className="mini-link ..."`，导致 mini-link
变体继续在业务页分散拼接。

### 根因

前期只给可触发动作的图标按钮建立了 `IconActionButton`，没有给 router link 形态的图标入口和 mini-link
入口建立对应原语。因此表格行 action 区和多个页面 command link 仍保留页面私有的链接样式路径。
...
- UI contract 增加防线，禁止 run table row icon link 回退为业务页手写 `icon-button dense`，并禁止 router
  mini-link 回退为 `Link className="mini-link"`。
...
- 新增 router 图标入口优先复用 `IconNavLink`，新增 router mini-link 入口优先复用 `InlineNavLink`；
  只有普通正文链接、外部/计算 href 或明确的业务导航卡片可以直接使用 `Link` / `<a>` 样式。
