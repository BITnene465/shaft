...
- 根因：历史增强按 bbox 跨度调强度，不能表示实际笔画/虚线间隔；通用量化去重未区分四点曲线合同。
- 影响范围：本轮 line 校准和即将生成的 v5.10 synthetic line；预览未作为训练数据发布。
  Shape 的既有全量验收是结构/媒体/复现验收，不能等同于视觉语义全量验收。
- 修复方式：按用户确认只选 JPEG 40–90（4:4:4）或 sigma 0.5–1.5 轻噪声，极小目标 clean，
  关闭 blur/重采样/叠加；v5.10 line 开启严格量化检查，碰撞 view 单独报告，不静默删点。
- 回归测试：JPEG 整数范围全覆盖、操作互斥、clean 保护、严格量化失败用例和微型真实构建/验收链。
- 后续防线：recipe/单独 policy id 作为采样真源，不改变既有 shape 策略；输出记录原 bbox 与 locator 边距；
  明确自动 schema/hash 验收不等于视觉语义保证，未知错误不得伪装成质量剔除继续发布。

## 2026-09-08：v5.10 跨机器数据复现合同

- 现象：用户要求整版数据可在其他开发机重建，仅记录 seed 无法证明输入与输出相同。
- 根因：旧 context builder 将来源目录 basename 写进训练行；同一源数据改名会改变内容。
  分层配额计算若依赖 set 遍历顺序，也可能受 Python hash seed 影响。
- 影响范围：本轮 v5.10 shape 生成与后续整版数据发布；不改变现有训练内核。
- 修复方式：显式 source snapshot id，排序稳定的配额/identity 选择，输入、代码、prompt、环境与
  输出哈希锁定；正式任务脚本/配置维护，staging 验收后替换，旧派生目录保留备份。
- 回归测试：配额顺序与容量测试、不同源目录同内容测试、跨 worker 数与 hash seed 的微型完整构建。
- 后续防线：整版 Git tag 只指向验收完成的提交；历史审核依赖必须交付，尚未确认的任务明确待办。
  逐字节媒体复现需匹配 NumPy/Pillow/codec 环境并最终比较 content hash，不能只比较样本量。

## 2026-09-08：V10 新版 GT 替换旧半尺寸标注

- 现象：旧 V10 GT 声明尺寸是原图一半；用户要求检查上游修复并替换正式 GT。
- 根因：本地旧下载 revision f01d8ec0b675e9885d8492ba10e48a1fa544fd62 已落后于上游
  df3f66d8e661b60641dc8d10a6f65d7a98dcaab8，新包修正了标注坐标和尺寸。
- 影响范围：100,500 份 GT；新版还存在数值取整差异及 2,923 处列表长度变化，不能假设只改 size。
- 修复方式：独立下载和 50 进程解压验证后替换 gt_standard，旧目录保留为
  gt_standard.before_df3f66d_20260908；更新目录 README，旧统计标记为历史快照。
- 回归测试：ZIP SHA256 匹配 HF metadata；50 个解压批次成功；100,500 份 JSON 可解析、ID 与 split
  并集一致、size 全部等于实际图片尺寸；逐层旧新比较保存在 gt_standard_update_validation.json。
- 后续防线：当前 GT 不得再套用二倍坐标补偿；旧尺寸/实例数统计需重跑，任务级几何质量仍在预处理阶段审核。

## 2026-09-08：V10 解压验证中的 AES 校验及二倍尺寸约定

- 现象：初版验证器将所有文件误报为 CRC 不一致；修正后发现图片实际宽高均为 GT `size` 的两倍。
- 根因：压缩包使用 WinZip AES AE-2，CRC 字段为零，完整性由解密认证校验；GT 声明尺寸与原图像素尺寸
  使用不同尺度，不能直接要求相等。尺寸差异不是解压损坏。
- 影响范围：V10 的三类文件各 100,500 个；全部 100,500 对图片/GT 的两个轴倍率均为 2。
- 修复方式：7-Zip 解压认证加落盘长度校验，独立 JSON 解析和完整图片解码；记录二倍尺度，不改原始图像或
  标注。用户授权在验证通过后删除原始压缩包。派生数据构建仍需显式适配原图坐标。
- 回归测试：301,500 个文件长度一致，201,000 份 JSON 可解析，100,500 张图片完整解码；三类 ID 完全
  对齐 train 100,000 和 val 500，split 无重合；二倍尺寸关系全量确认。任务属性质量尚未在此轮审核。
- 后续防线：按 ZIP 加密格式选择校验方式，不把 AE-2 零 CRC 当损坏；裁图、bbox、points 及坐标归一化前
  必须显式核对 GT 尺度，不能直接套用原图尺寸。记录见 V10 目录 README 与 extraction_validation.json。
