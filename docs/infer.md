...
- 当前仓库 `serve` extra 固定 `vllm==0.19.0`，其 Python 依赖约束仍是 `transformers<5`；
  因此 Qwen3.6 vLLM 服务建议先在独立镜像或外部服务环境验证，再通过 `vllm_openai` 接入 Shaft。
  如果只是做 HF 本地训练或处理器/模板 smoke，默认环境会解析到 Transformers 5.10.1；也可以使用
  `uv sync --extra dev --extra train --extra distributed --extra qwen-next --extra gpu` 显式固定
  新一代 Qwen 训练口径。`qwen-next` 与 `serve` 在依赖层声明为互斥，不能混装。
