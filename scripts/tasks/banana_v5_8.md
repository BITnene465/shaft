...
v5.8 若要让同一 canonical sample 学习属性 A、属性 B 和联合属性 AB，应使用 PromptSource 的
**task formulation sampling**：

```text
一条 source/structured 真值
  -> 一条 canonical SFT row（同时携带 A 与 B 的可重算参数）
  -> formulation A / B / AB
  -> 对应 prompt + 对应 target 原子产生
```

不要把同一图片预展开为三条 A/B/AB SFT 行。预展开会复制 row identity、放大数据体积，并让 curriculum、
resume 和 formulation 比例依赖离线展开方式。也不要只轮换 prompt 而复用同一个 AB target；这会制造问题与
监督不一致。
...
- structured row 保存任务完整语义，A、B、AB 都必须能从它或上游真源确定性重建。
...
### 4.1 v5.8 A/B/AB rendered-target 格式
...
- `sample_id` 在 dataset 内唯一且稳定；A/B/AB 不追加不同 id，因为它们不是三条 source row。
...
- `system_prompt`、`user_prompt`、`messages` 和 `target_text` 省略。写成空字符串虽可规范化，但新数据应直接
  省略，减少双重真源和歧义。
- `prompt_args` 中的 A/B 值必须由 structured/source truth 确定性计算；禁止把人工编辑过的最终答案只放在
  SFT 行中。
...
如果某个 v5.8 task 没有 A/B/AB 监督拆分，只需要旧式等义 prompt rotation，则继续提供 materialized
`target_text`：
...
1. `apply_to: train`：训练 JSONL 使用 `prompt_args`；val/test 准备完全 materialized 的固定 prompt + target，
   且不携带 `prompt_args`。若要分别报告 A/B/AB，每个 formulation 生成独立、稳定 id 的 eval row。
...
4. 对 A、B、AB 每个 formulation 重算 target，使用共享 codec 或 task validator 检查可解析性和 exact schema。
...
- [ ] A/B 的业务字段名和各自 exact output schema 已冻结；AB 是明确组合，不是字符串拼接猜测。
- [ ] 每个 canonical sample 的 A/B 都能从同一 structured/source truth 重算。
...
- [ ] A/B/AB target 全部能被对应 codec/validator 解析，键、枚举、坐标系和空值语义一致。
