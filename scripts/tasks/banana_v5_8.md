...
| `line_context_reconstruction` | `appearance, points, reconstruction` | 外观、点序列、完整重建 |
...
pool 中固定的 `sampling_weight` 做 weighted categorical sampling：grounding、shape 和 full-capable line
均使用 `1:1:4`；其中 `objects` 或 `reconstruction` 在对应 pool 内约占 66.7%。
...
dataset 和两个 pool。不要在一个 pool 内对单行动态禁用 formulation，也不要用 `null` 监督。
