...
_DIVERGENCES = frozenset({"forward_kl", "reverse_kl", "jsd"})
...
def _probability_dtype(tensor: torch.Tensor) -> torch.dtype:
    ...
    if tensor.dtype.is_floating_point:
        return tensor.dtype
    raise TypeError("OPD logits must use a floating-point dtype.")


@dataclass(frozen=True, slots=True)
class OPDLossComponents:
    numerator: torch.Tensor
    denominator: torch.Tensor


@dataclass(frozen=True, slots=True)
class OPDTeacherDistribution:
    ...
    def __post_init__(self) -> None:
        ...
        if kind not in {"dense_logits", "topk_tail"}:
            raise ValueError(f"Unsupported OPD teacher distribution kind={kind!r}.")
        if type(self.vocab_size) is not int or self.vocab_size <= 0:
            raise ValueError("OPD teacher distribution vocab_size must be > 0.")
    ...
    def _validate_dense(self) -> None:
        ...
        if self.dense_logits.ndim != 2:
            raise ValueError("OPD dense teacher logits must have shape [positions, vocabulary].")
        if int(self.dense_logits.shape[1]) != self.vocab_size:
            raise ValueError("OPD dense teacher logits disagree with vocab_size.")
        if not self.dense_logits.dtype.is_floating_point:
            raise TypeError("OPD dense teacher logits must be floating point.")
        if any(
            value is not None
            for value in (self.topk_token_ids, self.topk_log_probs, self.tail_log_probs)
        ):
            raise ValueError("Dense OPD teacher distribution cannot carry top-k fields.")
        if self.temperature is not None:
            raise ValueError("Dense OPD teacher distribution must not pre-bind temperature.")
    ...
    def _validate_topk(self) -> None:
        ...
        if self.topk_token_ids.ndim != 2 or self.topk_log_probs.ndim != 2:
            raise ValueError("OPD top-k fields must have shape [positions, k].")
        if tuple(self.topk_token_ids.shape) != tuple(self.topk_log_probs.shape):
            raise ValueError("OPD top-k token IDs and log probabilities must have identical shapes.")
        if self.topk_token_ids.dtype != torch.long:
            raise TypeError("OPD top-k token IDs must use torch.long.")
        if not self.topk_log_probs.dtype.is_floating_point:
            raise TypeError("OPD top-k log probabilities must be floating point.")
        top_k = int(self.topk_token_ids.shape[1])
        if top_k <= 0 or top_k > self.vocab_size:
            raise ValueError("OPD teacher top-k width must be in [1, vocab_size].")
        if self.topk_token_ids.numel() and (
            int(self.topk_token_ids.min().item()) < 0
            or int(self.topk_token_ids.max().item()) >= self.vocab_size
        ):
            raise ValueError("OPD top-k teacher token ID is outside vocabulary.")
        if top_k > 1:
            sorted_ids = self.topk_token_ids.sort(dim=-1).values
            if bool(sorted_ids[:, 1:].eq(sorted_ids[:, :-1]).any().item()):
                raise ValueError("OPD top-k teacher token IDs must be unique within each row.")
        temperature = float(self.temperature or 0.0)
        if not math.isfinite(temperature) or temperature <= 0:
            raise ValueError("OPD top-k teacher distribution requires a finite temperature > 0.")
        if top_k == self.vocab_size:
            ...
            if self.tail_log_probs is None:
                raise ValueError("Truncated OPD top-k distribution requires tail_log_probs.")
            if tuple(self.tail_log_probs.shape) != (int(self.topk_token_ids.shape[0]),):
                raise ValueError("OPD teacher tail_log_probs must have shape [positions].")
            if not self.tail_log_probs.dtype.is_floating_point:
                raise TypeError("OPD teacher tail_log_probs must be floating point.")
        ...
        if not bool(torch.isfinite(log_probs).all().item()):
            raise ValueError("OPD top-k teacher distribution contains non-finite log probabilities.")
        ...
        if not torch.allclose(total, torch.ones_like(total), atol=tolerance, rtol=tolerance):
            raise ValueError("OPD top-k teacher distribution is not normalized.")

    @classmethod
    def from_dense_logits(cls, logits: torch.Tensor) -> "OPDTeacherDistribution":
        if logits.ndim != 2:
            raise ValueError("OPD flattened dense teacher logits must be 2-D.")
        return cls(
            kind="dense_logits",
    ...
    def from_topk_logits(
        ...
        *,
        ...
        if logits.ndim != 2:
            raise ValueError("OPD flattened teacher logits must be 2-D for top-k projection.")
        top_k = int(top_k)
        vocab_size = int(logits.shape[-1])
        if top_k <= 0 or top_k > vocab_size:
            raise ValueError("OPD teacher top_k must be in [1, vocab_size].")
        temperature = float(temperature)
        if not math.isfinite(temperature) or temperature <= 0:
            raise ValueError("OPD teacher top-k temperature must be finite and > 0.")
...
@dataclass(frozen=True, slots=True)
class OPDObjectivePlan:
    mode: str
    backend_type: type["OPDObjectiveBackend"]
    divergence: str
    temperature: float
    top_k: int | None
    token_chunk_size: int | None

    def build(self) -> "OPDObjectiveBackend":
...
class OPDObjectiveBackend(ABC):
    name: str

    def __init__(self, plan: OPDObjectivePlan) -> None:
        if str(self.name).strip().lower() != plan.mode:
            raise ValueError("OPD objective implementation name does not match resolved plan.")
        self.plan = plan
    ...
    def compute(
        ...
        teacher_distribution: OPDTeacherDistribution,
    ) -> OPDLossComponents:
...
def _validate_flat_student_logits(
    ...
    if student_logits.ndim != 2:
        raise ValueError("OPD flattened student logits must have shape [positions, vocabulary].")
    if int(student_logits.shape[0]) != teacher_distribution.num_positions:
        raise ValueError("OPD student and teacher position counts differ.")
    if int(student_logits.shape[1]) != teacher_distribution.vocab_size:
        raise ValueError("OPD student and teacher vocabulary sizes differ.")
    if int(student_logits.shape[0]) <= 0:
        raise ValueError("OPD objective requires at least one completion position.")
...
def _divergence_from_log_probs(
    ...
    *,
    ...
    if tuple(student_log_probs.shape) != tuple(teacher_log_probs.shape):
        raise ValueError("OPD student/teacher log-probability shapes differ.")
    ...
    raise ValueError(f"Unsupported OPD divergence={divergence!r}.")


class FullVocabularyOPDObjective(OPDObjectiveBackend):
    name = "full_vocab"
    ...
    def build_teacher_distribution(
        ...
        *,
        ...
        _ = plan
        return OPDTeacherDistribution.from_dense_logits(teacher_logits)
    ...
    def validate_teacher_distribution(
        ...
        *,
        ...
        if distribution.kind != "dense_logits":
            raise ValueError("full_vocab OPD objective requires dense teacher logits.")
...
class TopKTailOPDObjective(OPDObjectiveBackend):
    name = "topk_tail"
    ...
    def build_teacher_distribution(
        ...
        *,
        ...
        if plan.top_k is None:
            raise ValueError("topk_tail OPD objective plan has no top_k.")
        return OPDTeacherDistribution.from_topk_logits(
            teacher_logits,
            top_k=plan.top_k,
    ...
    def validate_teacher_distribution(
        ...
        *,
        ...
        if distribution.kind != "topk_tail":
            raise ValueError("topk_tail OPD objective requires a topk_tail teacher distribution.")
        if distribution.top_k != plan.top_k:
            raise ValueError("OPD teacher top-k width differs from the objective plan.")
        if not math.isclose(
            ...
            raise ValueError("OPD teacher top-k temperature differs from the objective plan.")
...
class OPDObjectiveRegistry:
    def __init__(self) -> None:
        self._implementations: Registry[type[OPDObjectiveBackend]] = Registry("opd_objective")

    def register(
        self,
        name: str,
        implementation: type[OPDObjectiveBackend],
    ) -> type[OPDObjectiveBackend]:
        if not issubclass(implementation, OPDObjectiveBackend):
            raise TypeError("OPD objective implementations must subclass OPDObjectiveBackend.")
        normalized = str(name).strip().lower()
        if normalized != str(implementation.name).strip().lower():
            raise ValueError("OPD objective registration name differs from implementation name.")
        return self._implementations.register(normalized, implementation)

    def resolve(self, config: Any) -> OPDObjectivePlan:
        mode = str(config.mode).strip().lower()
        try:
            implementation = self._implementations.get(mode)
        except KeyError as exc:
            raise ValueError(
                f"Unknown OPD objective mode {mode!r}; registered={self._implementations.keys()}."
            ) from exc
        return OPDObjectivePlan(
            mode=mode,
...
OPD_OBJECTIVE_REGISTRY = OPDObjectiveRegistry()
OPD_OBJECTIVE_REGISTRY.register("full_vocab", FullVocabularyOPDObjective)
OPD_OBJECTIVE_REGISTRY.register("topk_tail", TopKTailOPDObjective)


def resolve_opd_objective_plan(config: Any) -> OPDObjectivePlan:
    return OPD_OBJECTIVE_REGISTRY.resolve(config)


def opd_distribution_loss(
    *,
    student_logits: torch.Tensor,
    teacher_logits: torch.Tensor,
...
) -> torch.Tensor | tuple[torch.Tensor, OPDLossComponents]:
    """Dense convenience entry using the canonical full-vocabulary objective backend."""

    if student_logits.shape != teacher_logits.shape:
        raise ValueError(
            "OPD student/teacher logits must have identical shapes, got "
            f"student={tuple(student_logits.shape)}, teacher={tuple(teacher_logits.shape)}."
        )
    if student_logits.ndim != 3:
        raise ValueError("OPD logits must have shape [batch, sequence, vocabulary].")
    if completion_mask.ndim != 2 or tuple(completion_mask.shape) != tuple(student_logits.shape[:2]):
        raise ValueError("OPD completion_mask must match the logits batch/sequence axes.")
    if student_logits.shape[1] < 2:
        raise ValueError("OPD requires at least two sequence positions for causal shift.")
    divergence = str(divergence).strip().lower()
    if divergence not in _DIVERGENCES:
        raise ValueError(f"Unsupported OPD divergence={divergence!r}.")
    temperature = float(temperature)
    if not math.isfinite(temperature) or temperature <= 0:
        raise ValueError("OPD temperature must be finite and > 0.")
    shifted_mask = completion_mask[:, 1:].to(device=student_logits.device, dtype=torch.bool)
    if not bool(shifted_mask.any().item()):
        raise ValueError("OPD batch has no valid completion tokens after causal shift.")
    ...
    plan = OPDObjectivePlan(
        mode="full_vocab",
        backend_type=FullVocabularyOPDObjective,
    ...
    components = plan.build().compute(
        flattened_student,
        OPDTeacherDistribution.from_dense_logits(flattened_teacher),
    )
