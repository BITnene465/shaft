...
def test_hf_sharded_checkpoint_validation_reports_incomplete_download(tmp_path: Path) -> None:
    model_dir = tmp_path / "model"
    model_dir.mkdir()
    (model_dir / "model-00001-of-00002.safetensors").write_bytes(b"")
...
def test_builder_rejects_incomplete_hf_sharded_checkpoint_before_loader(tmp_path: Path) -> None:
    model_dir = tmp_path / "model"
    model_dir.mkdir()
    (model_dir / "model-00001-of-00002.safetensors").write_bytes(b"")
    temp_dir = model_dir / "._____temp"
