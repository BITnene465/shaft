...
Dashboard 总览页是总控工作台，不展示 recall 这类精细评测指标；它只聚合 run 生命周期、评测覆盖、
数据规模、队列健康、服务运行态、Run/Job/Service 活动矩阵和最近 run 这些可行动粗信号。
总览视觉模块按可读控制台处理：ops、Run/Job/Service 三泳道 12 日期桶活动矩阵和实时遥测保留在 signal deck，
主区域使用分栏 masonry 信息流，默认 2 列，宽屏逐步扩展但最多 4 列；图表数量控制在 4-7 个高价值粗粒度 mini chart。
Notes、任务类型、模型分布、benchmark task、label footprint、样本/label 权重、Job 日历和 scheduler 资源不进入总览，
留在 Runs、Inspector、Rank Board 或 Services 的细节视图里。compact 视口下由图表矩阵自身滚动，不能被外层 hidden 裁切。最近 run 只作为图表矩阵里的事件 tile，不再用大块低信息密度 card 拉伸页面。高级检索 UI 已组件化为
...
Run/Job/Service 活动矩阵保持 3 条 12 桶泳道，mini chart 保持 4-7 个高价值粗信号且覆盖多种微图表形态，图表矩阵使用 capped masonry columns 而不是纯 grid，最多 4 列，不出现 precision/recall/IoU 这类细指标文案，也不能回流 Notes、Label footprint、模型分布、Job 日历或 Scheduler 资源这类低价值总览面板，
