...
- Eval 记录的用户可读主键是 `run_id`；`job_id` 只表示一次队列执行。评测中心和结果库的第一列都优先展示
  `run_id`，只有还没有落盘 run manifest 的 queued / failed job 才回退展示 `job_id`。
...
- Pixel budget 属于 eval/data 请求语义，不属于 vLLM runtime/service 启动语义；Eval Bench 在每次请求前按 Qwen smart resize 控制输入图像，不允许通过 runtime args 或 service extra args 设置 `--mm-processor-kwargs`、`--min-pixels`、`--max-pixels`。
...
- `runtime.args` 使用严格白名单；新增 vLLM 启动参数必须先进入 schema、payload 和命令生成逻辑。
- 未显式声明设备/TP 时，自动探测当前可见 GPU，按模型 attention heads 选择合法 TP，并把 placement 写入 resolved payload。
- 对未实现的 job kind 不创建队列记录，避免出现永远无法处理的任务。
  CLI `preflight-job` 的 JSON schema 显式声明 `resolved_manifest`、`resolved_payload` 和 nullable
  `runtime_command`；即使 job kind 不支持这类早期失败，也返回同一套 top-level 字段，agent 不需要
  为错误路径单独解析。job record 的 CLI schema 也会声明常用 `payload` 字段和 `metadata`
  进度 / preflight warning 字段，方便 agent 读取队列状态而不用理解 SQLite 内部结构。
  job template、prompt template、preflight 和 job record 共享同一套 manifest / generation / data
  schema，明确 `runtime.env`、`runtime.args`、`eval.generation`、`eval.data` 和 `preannotate`
  的主要字段，避免 agent 只能把模板和 resolved manifest 当作无结构 JSON。

Prompt 不再只靠用户手写 `prompt_id`。Dashboard 启动时会把 `configs/prompts/pools/grounding_layout.v2.4.yaml`、`grounding_arrow.yaml`、`grounding_shape.yaml`、`grounding_icon_image.yaml` 种子化到 SQLite prompt template registry，默认别名分别是：

- `grounding_layout.v2.4.main`
- `grounding_arrow.v2.4.main`
- `grounding_shape.v2.4.main`
- `grounding_icon_image.v2.4.main`
- `point_arrow.v2.4.main`

Jobs 页可以选择 job template 和 prompt template。默认 `eval_job` 是箭头检测任务；layout 检测保留为
`layout_eval_job` 模板。Banana v2.4 benchmark 同时维护 grounding detection slices 和 crop 级
`point_arrow` slice；但默认 job 模板只暴露 detection 入口，避免把 point 任务混入 grounding 主分。
需要跑 point eval 时应显式选择 `point_arrow` split、`point_arrow.v2.4.main` prompt 和 keypoint metric。
Prompt template 应用时会把
...
Keypoint job 和 keypoint prediction import 不暴露 label 子任务选择，默认只评价 `arrow` 关键点；CLI 查询
CLI `resolve-target-labels` 或 API `GET /api/target-labels` 时会返回
`label_subtasks_supported=false`，避免把 keypoint 误当成可任意 label 子集的 detection 子任务。
...
- `grounding_layout.v2.4.main` 只评价 `icon / image / shape`。
- `grounding_arrow.v2.4.main` 只评价 `arrow`。
...
prompt metadata、内置 suite 默认、task default、unscoped；这套来源会写入 run spec
metadata 并在 report 中保留，避免 prompt/template 推导出的 label 范围被误记为人工显式声明。
内置 suite 默认只匹配 Eval Bench 维护的任务族，例如 `grounding_layout.*`、`grounding_arrow.*`、
`grounding_shape.*`、`grounding_icon_image.*`、`point_arrow.*`
以及历史 `keypoint_arrow.*` / `arrow_keypoint.*`；自定义 prompt ID 即使包含 `layout` 这类普通词，
也不会被自动推断为 layout label 子任务，除非 prompt metadata 或 run spec 显式声明 `target_labels`。
`preflight-job` 的 `resolved_payload` 会应用同一套 label policy，让入队前就能看到最终评估范围。
...
```json
{
  ...
  "eval": {
    ...
    "data": {"max_pixels": 2000000, "batch_size": 1}
  }
}
```
...
如果 ephemeral vLLM job 或本地 service 没有显式声明 `CUDA_VISIBLE_DEVICES` / `tensor-parallel-size`，
Eval Bench 会探测当前可见 GPU，按模型 attention heads 选择合法的最大 tensor parallel size，
并把选中的设备写回 resolved payload / service config。scheduler 会基于 resolved 设备做排他，
避免大模型 job 因未声明设备而不占用 GPU。
...
- 独立 Rank Board：后端 `/api/rank-board` 和 CLI `rank-board` 提供同一份排序结果，支持 task、benchmark、status、label、model、prompt、metric、min score、全文 query、facet 计数和多种排序；返回的 `score_label` / `score` 始终对应当前主指标
...
- 将模型文本归一化为 prediction document 的 parser 工具；JSON 解析复用共享 `json_any` codec，
  并把 valid/partial/error/empty-repair 信号写入 prediction metadata
- 从持久化 prediction snapshot 生成报告的 `evaluate-run`；报告同时汇总 metric 和输出质量信号，
  包括 finish reason、截断、partial decode、空数组修复和 decode error 计数
...
`grounding_arrow`、`grounding_layout`、`grounding_shape`、`grounding_icon_image`、
`point_arrow`。Run manifest 里的 `benchmark.split`
决定本次评测消费哪个 slice；job/import 未显式声明 split 时，后端会按 task、prompt 和 target labels
推断内置 Banana slice。Eval Bench 内部只承认这些 task split，不把 raw 的 `part1` / `part2`
当作评测语义。raw 路径只作为 JSON `extra.source_json` / `extra.source_image` 溯源字段保留。

Banana v2.4 主评测使用 `scripts/tasks/create_banana_v2_4_eval_benchmark.py` 创建。grounding
slice 来自 raw full image；`point_arrow` slice 必须来自 `data/point_arrow/structured/val.jsonl`
的 arrow crop，每个样本对应一个 crop image，和 point-arrow SFT val 口径一致。grounding 主分数的
默认像素预算是 `max_pixels=2000000`；`max_pixels=1000000` 只作为低分辨率 stress eval，不作为主分。
数据重建方法论：

- point: 从 raw arrow `bbox + linestrip` 派生，每个有效 arrow 一个 bbox padding crop，padding ratio `0.2-0.5`，validation 使用稳定 padding，无 jitter。
- grounding: train 包含 full、少量 blur full、density crop、hard negative crop；clean full 与 blur full 总量约等于源 full 数量，blur 只是其中一部分。
- val 不增强：grounding val 保持 clean full image，point val 保持 deterministic crop。
- raw 是真源，structured / SFT / eval benchmark 都应可从 raw 和 split 文件重建。
...
.venv/bin/python scripts/eval_bench.py resolve-target-labels \
  --benchmark-id multitask_test_v1 \
  --prompt-id grounding_arrow.v2.4.main
...
.venv/bin/python scripts/eval_bench.py import-predictions \
  ...
  --prompt-id grounding_layout.v2.4.main \
  --target-label icon \
  --target-label image \
  --target-label shape
```
...
`grounding_layout.v2.4.main`、带 `metadata.target_labels` 的自定义 prompt template，或显式传
...
创建 job 或导入 prediction 之前，可以先用 CLI `resolve-target-labels` 或 Dashboard
API `GET /api/target-labels` 查询同一份 label policy：
...
三类 label 来源，以及拼错 label 时的 `valid=false` 和 errors；脚本化调用不需要直接读取 benchmark
artifact、prompt registry 或前端 manifest 状态。

Run note 入口是稳定 CLI，不需要直接改 store 文件：
...
取得的 `--expected-updated-at` 后会启用乐观并发校验，避免覆盖其他人或批处理任务刚写入的线索。
`append-run-note` 会追加带 heading
的结构化段落，适合脚本或批处理任务持续补充复现线索；也支持同一个 `--expected-updated-at`
...
生命周期操作也走稳定 CLI，不需要手改 SQLite 或移动 artifact 目录：
...
脚本化检索基础对象直接走标准 CLI，不需要读取前端 state 或手扫 store 目录。`list-benchmarks` /
`show-benchmark` / `list-runs` 共享 task、label、model、prompt、metric 和全文查询语义；job、service
和 comparison 列表同样通过 CLI 分页过滤，单个对象用对应的 `show-*` 命令读取。Run 初始化、prediction
文档校验和手动推进下一个 queued job 分别通过 `init-run`、`validate-prediction` 和 `process-next-job`
暴露；这些能力是普通子命令，不再额外提供 `list-agent-commands` / `show-agent-command` 这类 MCP 式
命令发现层。需要查看参数时使用标准 `--help`；需要机器可读失败时，在命令名前加 `--json-errors`
或设置 `EVAL_BENCH_JSON_ERRORS=1`，stderr 会输出
`{"ok": false, "command": "...", "error_type": "...", "message": "..."}`，并保留非零 exit code。
CLI 模块本身保持轻量 import；dashboard、worker、evaluator 和模型运行时依赖只在具体命令执行时懒加载。

```bash
.venv/bin/python scripts/eval_bench.py --help
.venv/bin/python scripts/eval_bench.py rank-board --help
...
.venv/bin/python scripts/eval_bench.py list-job-templates --query arrow
.venv/bin/python scripts/eval_bench.py show-job-template --template-id eval_job
.venv/bin/python scripts/eval_bench.py list-prompt-templates --task detection --query arrow
.venv/bin/python scripts/eval_bench.py show-prompt-template --prompt-id grounding_arrow.v2.4.main
.venv/bin/python scripts/eval_bench.py upsert-prompt-template --payload-file /path/to/prompt_template.json
.venv/bin/python scripts/eval_bench.py delete-prompt-template --prompt-id custom.arrow.v1
.venv/bin/python scripts/eval_bench.py list-benchmarks --task detection --layer layout --split val --query multitask
.venv/bin/python scripts/eval_bench.py show-benchmark --benchmark-id multitask_test_v1
.venv/bin/python scripts/eval_bench.py list-runs --task detection --benchmark-id multitask_test_v1 --label arrow --model-id qwen3vl-best --metric-profile detection_iou_v1 --query "prompt v2"
.venv/bin/python scripts/eval_bench.py list-comparisons --task detection --label arrow --baseline-run-id old_run --candidate-run-id new_run --query qwen3vl
.venv/bin/python scripts/eval_bench.py show-comparison --baseline-run-id old_run --candidate-run-id new_run
.venv/bin/python scripts/eval_bench.py show-comparison-sample --baseline-run-id old_run --candidate-run-id new_run --sample-index 0
.venv/bin/python scripts/eval_bench.py list-jobs --kind eval --status queued --query qwen3vl
.venv/bin/python scripts/eval_bench.py show-job --job-id eval_20260513_103418_ebb7f052
.venv/bin/python scripts/eval_bench.py list-services --kind local_vllm --status running --query qwen3vl
.venv/bin/python scripts/eval_bench.py show-service --service-id local-vllm-0
```

排障也走稳定 CLI，不需要直接读取 `eval_bench_store`。以下命令分别对应 dashboard
...
`filters` 以便复现 label / error 筛选与分页状态：
...
反推巡检合同。
Service lifecycle 命令也会声明 model service record 的稳定字段：`config` 覆盖 endpoint、本地
vLLM 启动参数和 extra args，`runtime` 覆盖 pid、command、endpoint、log path、启动/停止时间和
`health` 探测结果；agent 不需要读取 SQLite 或 Dashboard state 来推断服务结构。
...
Rank Board 默认按 `f1_iou50` 排序。`--sort-by precision_iou50|recall_iou50|mean_iou|prediction_count`
会同步更新返回的 `primary_metric`、`primary_metric_label` 和 entry `score`；`created_at` / `run_id`
只作为列表排序维度，主指标保持 F1。

需要查看参数直接运行 `rank-board --help`。`rank-board` 的 JSON stdout 包含分页字段、filters、
primary metric、`score_label`、七类 facets，以及 entries 中的
`rank/run_id/score/score_delta/status/benchmark/task/labels/model/prompt/metric_profile/note`。
调用方看到 `created_at` / `run_id` 时应理解为只改变列表顺序，默认主指标仍保持 `f1_iou50`。
...
合并主指标 F1 dial、当前最佳、报告闭环、待处理和运行压力四个可点击决策 tile，并用 Realtime Trace
展示报告覆盖、队列负载、服务容量、预测积压和 scheduler 资源占用。下方只保留评测闭环 runway 与最近 run 产物流，
...
首页主舞台不能使用“可以看排行”“查看排行榜”“等待报告进入排行”“主指标 F1 可排行”这类大号口号、
入口动作或可点击提示文案；已有报告时用报告数量、F1 状态、coverage 和具体 run 产物表达，而不是把“能去某页”
当作核心价值；即便卡片链接到 Rank Board，可见文本也只能表达报告评分状态，不能出现“可进入排序”这类导航式价值。
...
Notes、任务类型、模型分布、benchmark task、label footprint、样本/label 权重、Job 日历和 scheduler 资源明细面板不进入总览，
...
浮层内的 search / number / select 先写入 `AdvancedFilterBar` 草稿状态，点击“应用”或按 Enter 后才更新页面查询状态并刷新列表；存在未应用草稿时 filter 条显示 dirty 状态，应用按钮暴露 `aria-keyshortcuts=Enter`；
...
稳定分页 CLI 的 JSON schema 必须显式声明各自 `filters` key：Rank Board 覆盖 task/benchmark/status/label/model/prompt/metric/min_score/query，
run/benchmark/job/service/comparison 列表、样本列表和模板列表也要声明对应筛选字段，agent 不需要从样例 payload 反推筛选合同；
Rank Board 的排序入口必须收敛在表格表头，不再额外堆叠主指标下拉、排序按钮或 best eval 摘要；点击
F1、precision、recall、mIoU、预测数、创建时间或 Run ID 表头即可切换排序，重复点击当前表头切换升降序。
创建时间和 Run ID 只是列表排序字段，切回任一指标表头即可回到指标排名语义。高级检索只承载筛选条件和最低分门槛。
Rank Board 的高级检索下拉和 facet rail 都必须消费 `/api/rank-board` 返回的完整 `facets`，不能从当前页
entries、dashboard runs 或固定大页数请求推导可选目录。
Rank Board 页面使用后端 `offset/limit` 分页请求，不再只取固定前 200 条，翻页时保留当前筛选和排序；
每条 entry 还返回 `score_delta`，表示该 run 的
当前主分数与当前排序第一名的差值；分页后的条目仍使用完整排序榜首作为基准，便于判断差距。
Rank Board 表格必须常驻展示 F1、precision、recall、mIoU 和预测数列，当前排序列通过表头箭头和整列底色表达；
按指标排序时第一列使用 Rank 语义，按创建时间或 Run ID 排序时第一列降级为普通序号，避免把列表排序误读成模型质量排名。
`score_delta` 用于辅助解释当前排序下相对首行的分数差距，而不是作为独立排序控件。
...
Benchmark summary 会暴露 `labels`，供任务创建、检索 facet 和 CLI 统一消费，不要求前端扫描
benchmark 文件。
...
业务页和 viewer 控制区折叠面板使用 `DisclosurePanel`，run 配置、prompt 快照、prompt template、
...
Eval Bench 的基础 CI gate 在 `.github/workflows/eval-bench.yml`：backend contract 运行
`uv run pytest -q projects/eval_bench/tests -m "not smoke and not integration and not manual"`；
frontend contract 构建 dashboard，并运行 `npm run test:ui-contracts` 和 `npm run test:layout`。
真实 vLLM、GPU 长推理和 Docker build 不在默认 CI 中执行，业务推理环境一致性由
`docker/inference/README.md` 的 `shaft-contract-smoke` 做 integration/manual 验收。
...
Runs / Rank Board 的检索 smoke 会确认输入后出现 dirty 状态、应用按钮标注 Enter 快捷键，并通过 Enter
应用条件，避免回退到每个字符即时刷新整页；
...
Realtime Trace 只能作为 rank console 内的粗粒度状态 rail，不能退化为低价值图表墙；
不再回退到 signal stack、Run/Ops/Volume 面板组、proof/triage 堆叠或纯路由入口面板；
不出现 precision/recall/IoU 这类细指标文案，也不能回流 Notes、Label footprint、模型分布、Job 日历或 Scheduler 资源明细这类低价值总览面板，
...
Jobs 页还会检查 `.jobs-page` 不制造页面级全局滚动，队列表格由自己的容器滚动，detail sidebar 在桌面保持
full-height panel；small-height 视口下日志和详情只能内部压缩/滚动，不能撑破 workspace。
...
Rank Board contract 会检查 API/类型和前端使用 `score_label`，并阻止旧 `score_formula` 字段回流；
Jobs contract 会检查 `.jobs-page`、队列表格滚动边界和 detail sidebar 约束。
...
如果评测新一代 Qwen3.5 / Qwen3.6 并希望关闭 vLLM server 端 chat template 的 thinking 分支，可以在
`manifest.eval.inference_extra.extra_body` 中显式记录请求体扩展参数。Eval Bench 会把它透传给
OpenAI-compatible request，但不会允许它覆盖 `max_tokens`、`messages` 等核心字段，只有
`chat_template_kwargs` 这类 vLLM 扩展字段适合放在这里：
...
`extra_body` 不能携带 pixel budget 或 multimodal processor 参数，例如 `min_pixels`、
`max_pixels`、`mm_processor_kwargs` 以及它们的 kebab-case 形式。Eval Bench 会在请求前按
run manifest 的 `data.min_pixels/max_pixels` 做 Qwen smart resize；把这些字段透传给 vLLM 会让
不同 vLLM / Transformers 版本重新解释图像预算，导致业务推理与评测口径不一致。
...
`--cuda-visible-devices` 和 `--tensor-parallel-size` 可以省略；省略时会自动选择当前可见 GPU 和合法 TP。
...
`register-service`、`list-services`、`show-service`、`start-service`、`service-health`、`stop-service` 和
`delete-service` 的 CLI JSON schema 会显式声明 service record 的 `config`、`runtime`、`runtime.health`
和 `metadata` 结构，避免 agent 只拿到泛型 object 后再靠样例反推字段。
...
vLLM OpenAI 返回的 `finish_reason` 和 usage token 计数会写入每条 prediction metadata；当
`finish_reason=length` 或 completion tokens 触达 `max_tokens` 时，worker 会标记
`truncated_by_max_tokens`，供后续 report 区分“模型没找到目标”和“输出被截断/解析不完整”。
...
第一份 metric report 会写入 `runs/<run_id>/reports/metrics.json`，内容包括 IoU@0.50 precision/recall、mean IoU、per-label count、缺失 prediction file，以及 keypoint 任务下的 endpoint distance。检测任务默认使用 `detection_iou_v1` 的 per-label bbox IoU 贪心 matcher；需要全局最大匹配时显式选择 `detection_iou_maxmatch_v2`，它按 label 做最大基数二分匹配，并以匹配 IoU 总和作为次级目标。关键点任务使用 `keypoint_endpoint_v1` 的有序起终点距离 matcher，bbox IoU 只保留为诊断字段，不再决定 TP/FP/FN。它也会保存样本级诊断信息：TP/FP/FN 数量、match pair、per-sample mean IoU、false positive reference 和 false negative reference。report 还包含 `empty_prediction_rate`、`pred_gt_ratio`、per-label precision/recall、输出 char/token 长度分布、dense 样本分桶表现和输出质量计数：`finish_reason_counts`、`truncated_prediction_count`、`partial_decode_count`、`decode_empty_repair_count`、`decode_error_count`。`evaluate-run` 还会写入 `runs/<run_id>/reports/summary.json`，这是不含 sample list 的小型 dashboard index 文件，避免频繁刷新的 run table 解析完整 diagnostic report。
