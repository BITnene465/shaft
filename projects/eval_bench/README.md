...
- `overviewPage.tsx`：总控工作台页面；承载下一步动作、评测管线、运行压力、活动矩阵、行动入口和最近 run 摘要。
...
是否 repeatable 和互斥组要求；`mutates_state` 只是副作用标记，不是权限控制。新增 agent 命令必须同时进入
...
处理：左侧 focus panel 用下一步动作、Benchmarks/Predictions/Evaluated/Rank Ready 管线、Run/Ops/Volume
track rail 和 Run/Job/Service 三泳道 12 日期桶活动矩阵承载核心判断；右侧只保留行动入口和最近 run
的可点击紧凑摘要；不再维护分栏 masonry 图表墙，也不要求固定数量的 mini chart。
...
Run/Job/Service 活动矩阵保持 3 条 12 桶泳道，Overview 不能回流旧 mini chart wall 或 chart matrix；页面必须保留 next action、四段管线、行动入口、一个 focus panel 和最近 run 紧凑摘要组成的 command deck，
不出现 precision/recall/IoU 这类细指标文案，也不能回流 Notes、Label footprint、模型分布、Job 日历或 Scheduler 资源这类低价值总览面板，
关键入口必须有 hover/transition 反馈。Benchmark / Run 检查器还会模拟样本过滤后 0 命中的状态，确认侧栏、
