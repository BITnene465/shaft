...
也会记录这组 label。`target_labels` 的优先级是：run spec 显式声明、prompt metadata、内置 prompt
ID 推断。这样 layout prompt 在包含 arrow GT 的 benchmark 上评测时，不会把未要求输出的 arrow
误算成漏检；如果要做全任务评测，应显式使用包含所有目标 label 的 prompt/template，而不是只改
`task=detection`。外部 prediction snapshot 导入时也必须保留这组目标标签，不能只靠 `task=detection`
表达 layout 或 arrow 子任务。
