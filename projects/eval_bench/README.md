...
- `overviewPage.tsx`：总控工作台页面；承载当前判断、下一步动作、核心运行态、评测闭环和最近 run 摘要。
...
首页只回答三件事：当前是否可用、评测闭环卡在哪里、下一步应该去哪个页面。总览视觉模块采用 v11 flight deck：
顶部保持两列，左侧 priority stage 只放当前判断、同步状态、下一步主动作、关键规模和三个高频操作入口，
右侧 command rail 只放覆盖、待评、队列、服务四个可点击状态入口和运行态条形摘要。下方保持两列，
operations surface 承载基准样本 -> 预测产物 -> 评估报告 -> 排行就绪流线和 readiness switchboard，
最近 run 保持紧凑 ticker；不再单独常驻“阻塞优先级”面板，也不再使用只表达装饰关系的 orbit 图。
v11 根节点通过 pointer position CSS 变量驱动主舞台光照，主动作、状态卡、操作入口、pipeline stage 和最近 run
都必须有 hover / focus / transition 反馈；动效只增强可点击性、实时同步感和状态扫描，不引入前端私有业务语义。
最近 run 只展示 benchmark/model 与 prediction/report 数量。行动入口必须以 readiness switchboard
形态展示 service、queue、evaluation 和 rank board 四个状态入口，每个入口给出状态、数量占比和目标页面，不能退化成普通链接列表；最近 run 按
...
业务页折叠面板使用 `DisclosurePanel`，run 配置、prompt 快照、prompt template 和 weighted rank scheme 不再直接写
本地 `<details>/<summary>` shell；viewer 画布内的轻量 popover 可以保留专用外观，但不能复制业务页折叠面板层级。
...
Overview 不能回流旧活动矩阵、mini chart wall 或 chart matrix；页面必须保留 priority stage、command rail、hero next action、四段管线、readiness switchboard、一个 operations surface、运行态条形摘要和最近 run 紧凑 ticker 组成的 v11 flight deck surface，
...
清除动作也在这个边界内。Overview 的静态契约也由这里锁住：源码必须使用 v11 flight deck surface 的
`overview-home-v11`、`overview-command-center`、`overview-priority-stage`、`overview-command-rail`、
`overview-stage-map`、`overview-health-strip`、`overview-workbench` 和 `overview-ops-surface`，
并保留共享 hover / pulse 动效，不能重新引用旧
`overview-home-v6`、`overview-home-v7`、`overview-home-v10`、`overview-command-deck`、`overview-focus-panel`、`overview-side-stack`、
...
- 按钮层级、按钮图标和弹层表单网格规则见 `docs/eval_bench_ui_icon_design.md`；标准动作必须复用
  ...
  `controlPrimitives.tsx` 的 `CompactSelectControl` 或 `FormSelectControl`，业务页折叠面板必须复用
