...
Keypoint job 和 keypoint prediction import 不暴露 label 子任务选择，默认只评价 `arrow` 关键点；agent 查询
...
Agent 在创建 job 或导入 prediction 之前，可以先用 CLI `resolve-target-labels` 或 Dashboard
...
`resolve-target-labels` 和 `GET /api/target-labels` 明确返回 detection/keypoint label policy、
candidate labels、valid/errors/warnings；
...
`arrow` 且拒绝非 arrow label；agent 应先调用 `resolve-target-labels` 或 `GET /api/target-labels`
查看候选和最终范围，再执行会创建或导入 run 的命令。
