...
label、model、prompt、metric 和 note 全文查询语义；Runs、Benchmarks、Services 和 Rank Board 都必须走后端分页，筛选变化回到第一页，
不能在前端固定读取 200 条后截断；Benchmarks 页直接请求 `GET /api/benchmarks`，
和 CLI `list-benchmarks` 共享 task、layer、split 和全文查询语义；Services 页直接请求 `GET /api/services`，
