...
- `overviewPage.tsx`：总控工作台页面；承载下一步动作、评测管线、运行压力、阻塞优先级、readiness switchboard 和最近 run 摘要。
...
首页只回答三件事：当前是否可用、评测管线卡在哪里、下一步应该去哪个页面。总览视觉模块采用 v8 decision surface：
顶部只有左侧 priority stage 和右侧 command rail 两列；priority stage 把当前判断、同步状态、下一步主动作、
关键规模和基准样本 -> 预测产物 -> 评估报告 -> 排行就绪流线放在同一主线；command rail 只保留覆盖、待评、
队列、服务四个可点击状态入口。下方只保留一个 operations surface 承载 readiness switchboard，以及一个最近 run
紧凑摘要；不再单独常驻“阻塞优先级”面板，也不再使用只表达装饰关系的 orbit 图。最近 run 只展示
...
Overview 不能回流旧活动矩阵、mini chart wall 或 chart matrix；页面必须保留 priority stage、command rail、hero next action、四段管线、readiness switchboard、一个 operations surface 和最近 run 紧凑摘要组成的 decision surface，
...
清除动作也在这个边界内。Overview 的静态契约也由这里锁住：源码必须使用 decision surface 的
`overview-home-v8`、`overview-command-center`、`overview-priority-stage`、`overview-command-rail`、
