...
- `overviewPage.tsx`：总控工作台页面；承载下一步动作、评测管线、核心运行态、readiness switchboard 和最近 run 摘要。
...
`mutually_exclusive_groups`，便于 agent 区分只读查询、普通写入和删除/取消/停止这类危险生命周期操作，
...
`output_schema`，例如 `rank-board` 明确返回分页、filters、facets、主指标和 entry 字段，agent 不需要
读取内部 store 或猜测 JSON shape。`mutates_state`
...
首页只回答三件事：当前是否可用、评测管线卡在哪里、下一步应该去哪个页面。总览视觉模块采用 v9 mission-control surface：
顶部只有左侧 priority stage 和右侧 command rail 两列；priority stage 把当前判断、同步状态、下一步主动作、
关键规模和基准样本 -> 预测产物 -> 评估报告 -> 排行就绪流线放在同一主线；command rail 是实时信号板，只保留覆盖、待评、
队列、服务四个可点击状态入口。下方只保留一个 operations surface 承载 readiness switchboard，以及一个最近 run
紧凑 ticker；不再单独常驻“阻塞优先级”面板，也不再使用只表达装饰关系的 orbit 图。最近 run 只展示
benchmark/model 与 prediction/report 数量。行动入口必须以 readiness switchboard 形态展示 service、queue、evaluation
和 rank board 四个状态入口，每个入口给出状态、数量占比和目标页面，不能退化成普通链接列表；最近 run 按
...
留在 Runs、Inspector、Rank Board 或 Services 的细节视图里。compact 视口下由 Overview 页面栈承担滚动，核心面板不能被 hidden 裁切。首页和标准 workspace 元素保留入场、hover、active、状态 pulse、radar 和 rail 动效，这些动效只用于强调状态、可点击入口和实时感，不改变数据语义。高级检索 UI 已组件化为
...
Overview 不能回流旧活动矩阵、mini chart wall 或 chart matrix；页面必须保留 priority stage、command rail、hero next action、四段管线、readiness switchboard、一个 operations surface 和最近 run 紧凑 ticker 组成的 v9 mission-control surface，
...
清除动作也在这个边界内。Overview 的静态契约也由这里锁住：源码必须使用 v9 mission-control surface 的
`overview-home-v9`、`overview-command-center`、`overview-priority-stage`、`overview-command-rail`、
`overview-workbench` 和 `overview-ops-surface`，并保留共享 hover / pulse 动效，不能重新引用旧
`overview-home-v6`、`overview-home-v7`、`overview-command-deck`、`overview-focus-panel`、`overview-side-stack`、
...
- 按钮层级、按钮图标和弹层表单网格规则见 `docs/eval_bench_ui_icon_design.md`；标准动作必须复用
  `ui.tsx` 中的 `ActionButton`、`CommandButton`、`IconActionButton`、`IconNavLink`、`InlineNavLink` 或 `InlineAnchor`，可选行、chip 和 card 必须优先复用
