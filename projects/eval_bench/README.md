...
首页只回答三件事：当前是否可用、评测闭环卡在哪里、下一步应该去哪个页面。总览视觉模块采用 v10 cockpit：
...
Overview 不能回流旧活动矩阵、mini chart wall 或 chart matrix；页面必须保留 priority stage、command rail、hero next action、四段管线、readiness switchboard、一个 operations surface、运行态条形摘要和最近 run 紧凑 ticker 组成的 v10 cockpit surface，
...
清除动作也在这个边界内。Overview 的静态契约也由这里锁住：源码必须使用 v10 cockpit surface 的
`overview-home-v10`、`overview-command-center`、`overview-priority-stage`、`overview-command-rail`、
