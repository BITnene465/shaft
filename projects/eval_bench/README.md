...
首页只回答三件事：当前是否可用、评测管线卡在哪里、下一步应该去哪个页面。总览视觉模块按两列 home cockpit
处理：左侧 priority stage 把当前判断、同步状态、下一步主动作和基准样本 -> 预测产物 -> 评估报告 -> 排行就绪路线图放在同一主线；
右侧 command rail 只保留覆盖、待评、队列、服务四个可点击状态入口。下方只保留一个 operations surface 展示四段评测流向，
以及右侧 readiness switchboard 与最近 run 紧凑摘要；不再单独常驻“阻塞优先级”面板。最近 run 只展示
...
Overview 不能回流旧活动矩阵、mini chart wall 或 chart matrix；页面必须保留 priority stage、command rail、hero next action、四段管线、readiness switchboard、一个 operations surface 和最近 run 紧凑摘要组成的 home cockpit，
...
清除动作也在这个边界内。Overview 的静态契约也由这里锁住：源码必须使用 home cockpit 的
`overview-home-v7`、`overview-home-shell`、`overview-priority-stage`、`overview-command-rail`、`OverviewHeroMap`、
`overview-workbench` 和 `overview-ops-surface`，并保留共享 hover / pulse 动效，不能重新引用旧
`overview-home-v6`、`overview-command-deck`、`overview-focus-panel`、`overview-side-stack`、阻塞优先级面板或活动矩阵组件。
