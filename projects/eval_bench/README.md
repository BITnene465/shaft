...
显式加权排行只在用户显式传入 `rank_scheme` 时启用；默认 Rank Board 不做加权，仍按
...
只作为列表排序维度，主指标保持 F1。`--sort-by weighted_score` 只有在同时提供
`--rank-scheme-json` 或 `--rank-scheme-file` 时才合法；权重项必须声明 benchmark、metric、weight
和缺失指标处理规则：
...
`rank/run_id/score/score_delta/status/benchmark/task/labels/model/prompt/metric_profile/note/score_components`。
