...
label、model、prompt、metric 和 note 全文查询语义；Runs、Benchmarks 和 Rank Board 都必须走后端分页，筛选变化回到第一页，
...
关键入口必须有 hover/transition 反馈。Runs 页必须暴露 `.run-list-pager` 并通过 `/api/runs?offset&limit`
分页，Benchmarks 页必须暴露 `.benchmark-list-pager` 并通过 `/api/benchmarks?offset&limit` 分页，
