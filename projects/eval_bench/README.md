...
- `overviewPage.tsx`：总控工作台页面；承载两列 command deck、评测推进轨道、运行压力、活动矩阵和最近 run 摘要。
...
首页只保留评测推进、待评估 backlog、队列/服务压力、样本/预测规模、Run/Job/Service 三泳道 12 日期桶活动节奏和最近 run。
总览视觉模块按两列 command deck 处理：左侧 focus panel 用 progress rail、track rail 和活动矩阵承载核心判断，
右侧只保留最近 run 的可点击摘要；不再维护分栏 masonry 图表墙，也不要求固定数量的 mini chart。
...
Run/Job/Service 活动矩阵保持 3 条 12 桶泳道，Overview 不能回流旧 mini chart wall 或 chart matrix；页面必须保持一个 focus panel 加一个最近 run panel 的两列 command deck，
