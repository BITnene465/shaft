...
- `overviewPage.tsx`：总控工作台页面；承载高密度粗粒度图表、运行态遥测和最近 run 摘要。
...
- `controlPrimitives.tsx`：设置页、viewer 和 manifest toolbar 共用的 number input、color input、select、toggle 基础控件。
...
发现模板、筛选任务类型，并按需维护 prompt template registry。CLI 模块本身保持轻量 import；
dashboard、worker、evaluator 和模型运行时依赖只在具体命令执行时懒加载，避免 agent 的检索入口被
重型运行时拖慢：
...
`f1_iou50` 排序。权重项必须声明 benchmark、metric、weight 和缺失指标处理规则：
...
任务类型、模型分布、Prompt 分布、benchmark task、label footprint、样本规模、数据层、split、label scope、
run 新鲜度、连续 12 个日期桶的紧凑写入节奏、活跃任务、队列、worker service 和 scheduler 粗粒度状态。
总览视觉模块按高密度控制台处理：写入节奏使用微型柱状条，主区域压缩为 12 个环形占比 + 条形图 mini chart，
最近 run 只作为窄事件轨，不再用大块低信息密度 card 拉伸页面。高级检索 UI 已组件化为
...
绕过标准 action button；Settings 页的局部 select、搜索清空、重置和 label 清除动作也在这个边界内。
...
- 按钮层级、按钮图标和弹层表单网格规则见 `docs/eval_bench_ui_icon_design.md`；标准动作必须复用
  `ui.tsx` 中的 `ActionButton`、`CommandButton` 或 `IconActionButton`，局部 select 必须优先复用
  `controlPrimitives.tsx`。长路径/URL 字段必须使用
