...
模型分布、Prompt 分布、连续 12 个日期桶的紧凑写入节奏、活跃任务、队列、worker service 和 scheduler
粗粒度状态。总览视觉模块按高密度控制台处理：写入节奏使用微型柱状条，分类信号使用环形占比 + 条形图矩阵，
最近 run 使用内容自适应紧凑行，不再用大块低信息密度 card 拉伸页面。高级检索 UI 已组件化为
...
显式 weighted scheme 会返回 `weighted_score`、`rank_scheme` 和每条 entry 的 `score_components`，便于 agent
解释最终分。
