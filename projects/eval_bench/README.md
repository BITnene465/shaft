...
Prompt 不再只靠用户手写 `prompt_id`。Dashboard 启动时会把 `configs/prompts/grounding_layout.yaml`、`grounding_arrow.yaml`、`grounding_shape.yaml`、`grounding_icon_image.yaml`、`point_arrow.yaml` 种子化到 SQLite prompt template registry，默认别名分别是：

- `grounding_layout.latest`
- `grounding_arrow.latest`
- `grounding_shape.latest`
- `grounding_icon_image.latest`
- `point_arrow.latest`
...
- `point_arrow.latest` 只评价 `arrow`，并额外评价关键点。
...
.venv/bin/python scripts/eval_bench.py show-job-template --template-id keypoint_eval_job
.venv/bin/python scripts/eval_bench.py list-prompt-templates --task detection --query arrow
.venv/bin/python scripts/eval_bench.py show-prompt-template --prompt-id grounding_arrow.latest
