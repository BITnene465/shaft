...
Prompt 不再只靠用户手写 `prompt_id`。Dashboard 启动时会把 `configs/prompts/grounding_layout.yaml`、`grounding_arrow.yaml`、`grounding_shape.yaml`、`grounding_icon_image.yaml`、`grounding_shape_arrow.yaml`、`point_arrow.yaml` 种子化到 SQLite prompt template registry，默认别名分别是：

- `grounding_layout.latest`
- `grounding_arrow.latest`
- `grounding_shape.latest`
- `grounding_icon_image.latest`
- `grounding_shape_arrow.latest`
- `point_arrow.latest`
...
`grounding_shape.*`、`grounding_icon_image.*`、`grounding_shape_arrow.*`、`point_arrow.*`
