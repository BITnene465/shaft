...
首页只回答三件事：当前是否可用、评测闭环卡在哪里、下一步应该去哪个页面。总览视觉模块采用 v12 two-band control surface：
顶部两列只放主判断区和运行信号区，主判断区承载同步状态、下一步动作和 benchmark -> prediction -> report -> rank 的闭环 spine，
运行信号区只放报告覆盖、待评估、任务队列和模型服务四个可点击信号。底部两列只放最近 run 产物流和下一步工作区入口，
不再维护 readiness switchboard、低频诊断面板、分栏 masonry 图表墙或固定数量 mini chart。
v12 根节点通过 pointer position CSS 变量驱动主舞台光照，主动作、状态卡、闭环节点、工作区入口和最近 run
...
Overview 不能回流旧活动矩阵、mini chart wall 或 chart matrix；页面必须保留 v12 two-band surface：主判断区、下一步动作、四段闭环 spine、运行信号区、最近 run 产物流和下一步工作区入口，
并保留报告覆盖、待评估、任务队列和模型服务四个可点击信号，不再回退到 Run/Ops/Volume 面板组；
...
清除动作也在这个边界内。Overview 的静态契约也由这里锁住：源码必须使用 v12 two-band surface 的
`overview-home-v12`、`overview-workband`、`overview-hero-board`、`overview-flow-spine`、
`overview-signal-board`、`overview-recent-card` 和 `overview-route-panel`，
