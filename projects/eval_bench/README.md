...
首页只回答三件事：当前状态是否可行动、F1 主指标与闭环覆盖是否成立、最近产物是否在推进。总览视觉模块采用 v13 value-first command console：
顶部保持两列以内，左侧主判断区承载同步状态、下一步动作和 F1/覆盖/backlog 三个 proof metric，右侧 live panel
只放报告覆盖、待评估、任务队列、模型服务四个可点击信号和 triage rail。下方只保留评测闭环 spine 与最近 run 产物流，
不再维护 readiness switchboard、低频诊断面板、分栏 masonry 图表墙、固定数量 mini chart 或纯导航入口面板。
v13 根节点通过 pointer position CSS 变量驱动主舞台光照，主动作、状态卡、proof metric、闭环节点、triage 入口和最近 run
...
Overview 不能回流旧活动矩阵、mini chart wall 或 chart matrix；页面必须保留 v13 command console：主判断区、下一步动作、F1/覆盖/backlog proof metric、四段闭环 spine、运行信号区、triage rail 和最近 run 产物流，
并保留报告覆盖、待评估、任务队列和模型服务四个可点击信号，不再回退到 Run/Ops/Volume 面板组或纯路由入口面板；
...
清除动作也在这个边界内。Overview 的静态契约也由这里锁住：源码必须使用 v13 command console 的
`overview-home-v13`、`overview-command-shell`、`overview-now-panel`、`overview-proof-strip`、
`overview-live-panel`、`overview-loop-panel`、`overview-flow-spine`、`overview-triage-rail` 和 `overview-recent-card`，
并保留共享 hover / pulse 动效，不能重新引用旧
`overview-home-v6`、`overview-home-v7`、`overview-home-v10`、`overview-home-v12`、`overview-command-deck`、`overview-focus-panel`、`overview-side-stack`、
