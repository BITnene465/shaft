...
最近 run 使用压缩 run stream，只展示 benchmark/model、prediction/report/note 产物信号、创建时间和状态，不展示 P/R/IoU 细指标。行动入口必须以 readiness switchboard
...
最近 run 必须使用产物完成度 rail 与状态胶囊表达，不退回只堆 run id 的普通结果列表；关键入口必须有 hover/transition 反馈；compact / narrow 视口下 Overview focus、readiness 和 recent
