...
- `overviewPage.tsx`：总控工作台页面；承载可读分栏粗粒度图表、运行态遥测和最近 run 摘要。
...
- `sampleNavigation.ts`：样本 URL query、分页 offset、offset 合法化和相邻样本导航。
...
任务类型、模型分布、benchmark task、label footprint、样本规模、样本权重、时间节奏、job、service 和 scheduler 粗粒度状态。
总览视觉模块按可读控制台处理：ops、Run/Job/Service 三泳道 12 日期桶活动矩阵和实时遥测保留在 signal deck，
主区域使用分栏 masonry 信息流，默认 2 列，宽屏逐步扩展但最多 4 列；图表数量控制在 8-16 个高价值粗粒度环形占比、柱状、栅格、堆叠条、sparkline 和 mosaic mini chart，只保留能辅助调度判断的运行态、数据规模、覆盖进度、队列/服务和近期写入信号。compact 视口下由图表矩阵自身滚动，不能被外层 hidden 裁切。最近 run 只作为图表矩阵里的事件 tile，不再用大块低信息密度 card 拉伸页面。高级检索 UI 已组件化为
...
Run/Job/Service 活动矩阵保持 3 条 12 桶泳道，mini chart 保持 8-16 个且覆盖 6 种微图表形态，图表矩阵使用 capped masonry columns 而不是纯 grid，最多 4 列，不出现 precision/recall/IoU 这类细指标文案，
