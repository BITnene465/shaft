...
assert.equal(SELECT_VISIBLE_LIMIT, 80, "select popover must keep the bounded render window");

const centeredStart = centeredSelectWindowStart(120, options.length);
assert.equal(centeredStart, 80, "centered window should place the active item in context");
...
assert.equal(visibleWindow.start, 80, "window start should respect the requested stable start");
assert.equal(visibleWindow.hiddenBefore, 80, "window must report hidden items before it");
assert.equal(visibleWindow.hiddenAfter, 80, "window must report hidden items after it");
...
assert.equal(negativeStartWindow.hiddenAfter, 160, "clamped leading window must report hidden tail");

const oversizedStartWindow = selectVisibleWindow(options, 999);
assert.equal(oversizedStartWindow.start, 160, "oversized window starts must clamp to the final page");
assert.equal(oversizedStartWindow.hiddenBefore, 160, "final page must report hidden leading items");
assert.equal(oversizedStartWindow.hiddenAfter, 0, "final page must not report hidden trailing items");

assert.equal(
  selectWindowStartForActiveIndex(80, 120, options.length),
  80,
  "active item inside the window must not move the stable window"
);
assert.equal(
  selectWindowStartForActiveIndex(80, -1, options.length),
  80,
  "negative active item must keep the stable window"
);
assert.equal(
  selectWindowStartForActiveIndex(80, 79, options.length),
  79,
  "active item above the window should move the window just enough"
);
assert.equal(
  selectWindowStartForActiveIndex(80, 160, options.length),
  81,
